...
packages/    @deepseek-ai/dsh-<pkg> workspaces at packages/<group>/<pkg>/
  ...
  evolution/            evolutionary harness: controller, memory, lineage, scorers, tools
  runtime/              agent kernel and request-context runtime
  code-runtime/         code-execution runtimes (worker thread, python)
...
- Never default to the full suite or repeat a passing check for commit or push. CI owns exhaustive coverage and the platform matrix; rehearse all locally only by explicit request, for CI diagnosis, or for an irreducibly repository-wide change; `check:all` is the repository-wide fallback until the GitLab pipeline proves green.
