...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-evolution': { kind: 'none', reason: 'Browser journey surface plus the host-side curator status Remote face; the page reads recorded evolution state and the counters, and registers nothing model-facing.' },
  'packages/client/ui-workspace': { kind: 'none', reason: 'Browser-side UI plugin layer; registers nothing model-facing.' },
  'packages/workspace/workspace-memory': { kind: 'indirect', reason: 'The store holds the durable record and capacity accounting; dsh-workspace-memory-context owns the model-visible brief rendering.' },
  'packages/evolution/evolution-memory': { kind: 'indirect', reason: 'The store holds the durable record and capacity accounting; dsh-evolution-memory-context owns the model-visible brief rendering.' },
  'packages/skill/evolution-skill-telemetry': { kind: 'indirect', reason: 'The store holds per-skill usage counters and curation state; dsh-evolution-skill-manage owns the model-visible skill_manage tool.' },
  'packages/llm/llm-fallback': { kind: 'indirect', reason: 'The plugin selects the serving route on recovery; request assembly and adapters own the model-visible request.' },
  'packages/client/ui-workspace-memory': { kind: 'indirect', reason: 'The page edits the stored record through the workspaceMemory Remote; dsh-workspace-memory-context owns the model-visible brief rendering.' },
  ...
  'packages/client/ui-desktop-notifications': { kind: 'none', reason: 'Desktop-only OS notification watcher over already-published Session and job state; registers no tool, prompt section, or session event.' },
  ...
  'packages/client/ui-progress': { kind: 'none', reason: 'Browser-only progress panel over the todo projection and produced-file Turn data; registers no tool, prompt section, or session event.' },
  ...
  'packages/client/ui-settings-authorization': { kind: 'none', reason: 'Browser-side sign-in companion under the Models page seat; it renders Host authorization frames and registers no prompt, tool, schema, or session event.' },
  ...
  'packages/client/ui-kernel-task': { kind: 'none', reason: 'The node renders persisted kernel events and adds no prompt content of its own.' },
  'packages/bundle/agent-governance': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; the kernel and guard packages own every model-visible effect of the rows it inserts.' },
  'packages/bundle/kernel-ops': { kind: 'none', reason: 'The commands run outside any agent, reach no model, and append no session events.' },
  ...
  'packages/llm/embeddings': { kind: 'none', reason: 'The registry forwards caller-supplied texts to the registered provider and adds no content of its own, so it registers nothing model-facing.' },
  ...
  'packages/credentials/authorization-remote': { kind: 'none', reason: 'The Remote namespace serves credential sign-in conversations to browser surfaces; it registers no prompt, tool, schema, or session event.' },
  ...
  'packages/evolution/evolution-heartbeat': { kind: 'none', reason: 'The scheduler registers nothing model-facing; a registered task owns every model-visible effect it produces.' },
  'packages/evolution/evolution-dreaming': { kind: 'none', reason: 'The cycle scores and promotes observations the feedback seam already recorded; it registers no prompt, tool, schema, or session event.' },
  'packages/evolution/evolution-feedback': { kind: 'none', reason: 'The failure store registers nothing model-facing; a consumer rendering recorded failures owns that request.' },
  'packages/evolution/evolution-scorer': { kind: 'none', reason: 'The scorer reads recorded fixtures and reduces them to a measurement triple; it registers no prompt, tool, schema, or result text.' },
  'packages/evolution/evolution-self-model': { kind: 'none', reason: 'The self-model store keeps per-skill assessments and capability entries; it registers no prompt, tool, schema, or session event, and a consumer rendering the frontier owns that request.' },
  'packages/evolution/evolution-uncertainty': { kind: 'none', reason: 'The uncertainty store keeps recorded signals and derives queue rows from them; it registers no prompt, tool, schema, or session event, and a consumer rendering the queue owns that request.' },
  'packages/evolution/evolution-lineage': { kind: 'none', reason: 'The lineage store keeps dependency-versioned experiment envelopes; it registers no prompt, tool, schema, or session event, and a consumer rendering envelopes owns that request.' },
  'packages/evolution/evolution-sleeptime': { kind: 'none', reason: 'The sleeptime store keeps anticipated tasks and precompute artifacts; it registers no prompt, tool, schema, or session event, and a consumer rendering a plan owns that request.' },
  'packages/evolution/evolution-actuator': { kind: 'none', reason: 'The heartbeat loops act on recorded verdicts and write only to the stores that recorded them; nothing here enters a model request, and the request a promoted skill or admitted task later appears in belongs to its own owner.' },
  'packages/evolution/evolution-adversary': { kind: 'none', reason: 'The adversary store keeps recorded probes and derived defense rows; it registers no prompt, tool, schema, or session event, and a consumer rendering a challenge owns that request.' },
  'packages/evolution/evolution-benchmark': { kind: 'none', reason: 'The benchmark store keeps evaluation tasks, their contamination states, and their dedupe hashes; it registers no prompt, tool, schema, or session event, and a runner executing a task owns that request.' },
  'packages/evolution/evolution-budget': { kind: 'none', reason: 'The budget store keeps allocations, spends, and the screening schedule; it registers no prompt, tool, schema, or session event, and a consumer rendering an allocation owns that request.' },
  'packages/evolution/evolution-canary': { kind: 'none', reason: 'The rollout store keeps deployment states, measured triples, and the risk route; it registers no prompt, tool, schema, or session event, and the request a promoted artifact later appears in belongs to its own owner.' },
  'packages/evolution/evolution-curriculum': { kind: 'none', reason: 'The curriculum store stages grounded training proposals; it registers no prompt, tool, schema, or session event, and a consumer rendering a proposal owns that request.' },
  'packages/evolution/evolution-evaluator-health': { kind: 'none', reason: 'The health store keeps recorded verdicts and their agreement, drift, and error-rate readings; it registers no prompt, tool, schema, or session event.' },
  'packages/evolution/evolution-evaluator-strategy': { kind: 'none', reason: 'The strategy store keeps per-evaluator trust and judge independence; it registers no prompt, tool, schema, or session event, and a consumer choosing an evaluator owns that request.' },
  'packages/evolution/evolution-islands': { kind: 'none', reason: 'The island store keeps lanes, migration records, and due schedules; it registers no prompt, tool, schema, or session event.' },
  'packages/evolution/evolution-meta': { kind: 'none', reason: 'The meta store keeps engine runs under their configurations and recommends one; it registers no prompt, tool, schema, or session event, and nothing here changes what the engine runs.' },
  'packages/evolution/evolution-metrics': { kind: 'none', reason: 'The metric layer reads the stores and renders a report to the operator; no metric enters a prompt, a tool result, or a session event.' },
  'packages/evolution/evolution-model-routes': { kind: 'none', reason: 'The route store keeps per-role assignments and their measured evidence; it registers no prompt, tool, schema, or session event, and nothing here routes a run.' },
  'packages/evolution/evolution-novelty-search': { kind: 'none', reason: 'The novelty archive keeps behavior descriptors and derived archive novelty; it registers no prompt, tool, schema, or session event.' },
  'packages/evolution/evolution-operators': { kind: 'none', reason: 'The operator store keeps attempts, acceptance, delta, and regression statistics per operator and artifact class; it registers no prompt, tool, schema, or session event, and nothing here rewrites an operator instruction.' },
  'packages/evolution/evolution-population': { kind: 'none', reason: 'The population store keeps per-skill candidates with their lineage and status; it registers no prompt, tool, schema, or session event.' },
  'packages/evolution/evolution-retrieval': { kind: 'none', reason: 'The retrieval store keeps configurations and the downstream outcomes of the sessions that ran under them; it registers no prompt, tool, schema, or session event, and the producer records beside the turn rather than inside it.' },
  'packages/evolution/evolution-router': { kind: 'none', reason: 'The routing store keeps measured outcomes and derived effectiveness per task class and role; it registers no prompt, tool, schema, or session event, and nothing here routes a run.' },
  'packages/evolution/evolution-stagnation': { kind: 'none', reason: 'The stagnation store keeps evaluation runs and the derived strategy; it registers no prompt, tool, schema, or session event, and a consumer acting on a rung owns that effect.' },
  'packages/evolution/evolution-trace': { kind: 'none', reason: 'The trace store projects the committed session log read-only; it registers no prompt, tool, schema, or session event of its own.' },
  'packages/evolution/evolution-verifiers': { kind: 'none', reason: 'The verifier ladder judges a candidate body against mounted seams and returns a verdict to its caller; it registers no prompt, tool, schema, or session event.' },
