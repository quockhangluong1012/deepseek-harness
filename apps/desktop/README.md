...
The local startup page exposes startup status and available recovery actions; the loaded dsh renderer receives only the desktop protocol marker. The separate plugin window receives structured list, install, remove, update, and update-check operations; neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments. An `http` or `https` link a renderer opens — a `_blank` target or a navigation to another page — is handed to the operating system's browser instead of becoming a second window or replacing the application document; every other protocol reaches no handler.
