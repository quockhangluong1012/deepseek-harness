...
渲染进程打开的 `http` 或 `https` 链接——`_blank` 目标或跳转到其他页面——会交给操作系统浏览器，而不会成为第二个窗口或替换应用文档；任何其他协议都不会到达任何处理器。
...
Workspace 开发通过开发版应用菜单的“刷新页面”操作重载渲染层，而不是重启外壳。与 `dev:desktop` 并行运行 `pnpm run dev:web`，即可在保存时重建客户端 bundle 和 Web 前端产物。`apps/desktop/src` 下的修改仍需要执行 `pnpm run build:desktop` 并通过 `start:desktop` 重新启动，因为 Electron 加载的是已构建的主进程。已安装的应用只提供不可变 bundle。
