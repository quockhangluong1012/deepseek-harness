...
// Full-text session search ships off at the shared base layer (`openAt:
// never`) but on for Web/Desktop (`openAt: first-search` with a durable
// path): both openings defer the `node:sqlite` import until the first
// search call, so plain startup never emits the SQLite experimental warning.
...
describe.skipIf(!requireBuiltArtifacts)('built CLI lazy-search startup', () => {
  it('boots and disposes the shipped composition with full-text search deferred to first use', async () => {
    ...
    // The web restatement opts into content search with a durable index, deferred to first search.
