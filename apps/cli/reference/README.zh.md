...
报告包含 DSH 和 Node 版本、平台、profile、根配置路径、每个未激活插件的模块与状态、原始错误，以及启动期间的警告和错误参数，包括尚无 Fiber 的导入错误。报告还带有一个 `doctor` 数组，记录启动失败后仍能运行的部署检查：harness home 所在卷的空闲字节数，以及沙箱、提供方密钥、MCP 与语言服务器检查明确给出的 `unavailable`，各项都点明该失败导致不可达的服务。在该数组中，凭据只以引用名出现。Node 检查输出保留嵌套原因、聚合成员、循环引用、不可枚举属性和 Symbol 属性，并关闭深度、字符串及数组长度限制。自定义检查函数被禁用，访问器只描述而不求值。收集器包含失败后的异步清理日志，并在启动结算后停止。它不会独立于已记录错误额外收集环境变量或配置内容。插件原始错误可能包含配置或凭据值；报告开头会提醒读者在分享前检查内容。报告不脱敏。
