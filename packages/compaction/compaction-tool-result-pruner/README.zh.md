...
`dsh-compaction-tool-result-pruner` 在摘要前限制超大工具结果。它用配置的头部、中间标记和尾部替换符合条件的文本，保留非文本块，并记录精确回放引用。挂载 `dsh-agent-context` 后，编译器选中的近期结果与含 spill 恢复提示的结果会保持完整；其他超阈值结果会被修剪。它不发起模型调用；减少 token 压力可能让摘要无需运行。字符预算只能近似 token 用量；token meter 判断压力是否得到缓解。
...
只有未被 `dsh-agent-context` 最近一次实时放置选中的超阈值工具结果会被修剪。已放置的近期结果来源与每个必需的 spill 提示来源都会保留其完整结果；没有成功的实时编译时，每个超阈值的当前表面结果都可被修剪。图片与结构化块等富内容保持原有顺序，并保留日志中的所有图片省略选择。替换会保留工具调用、步骤、错误与元数据——只有文本内容会变化。替换无法记录时运行会失败，先前完成的修剪仍会保留。
