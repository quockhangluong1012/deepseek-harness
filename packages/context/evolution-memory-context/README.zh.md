...
| `minSupersedeChangeBytes` | `0` | 替换可见简报前，渲染文本必须变化的最小字节数（S1 第 4 点）|
| `maxSupersedesPerSession` | `1000000` | 单会话简报替换次数上限；超出后存储变更不再替换可见简报 |
...
存储摘要变化本身不会替换可见简报：注入器会逐字节比较新渲染文本与上一次放到界面上的文本。一次让渲染文本保持不变的存储写入——例如经验 `confirms` 决策只会递增 `validationCount`——即便存储自身的摘要变了，也绝不会触发替换。一次真正的文本变化仍需至少 `minSupersedeChangeBytes` 个不同字节，并且该会话在 `maxSupersedesPerSession` 之下还有未用完的额度；只要有一项不满足，可见简报就原样保留，请求前缀也保持缓存稳定。被拦下的变化不会丢失：下一次存储变化仍以同一份仍在界面上的文本为基准比较，因此多次小改动会累积逼近阈值，而不是每次都重新归零。
...
三个提示注册：`evolution-memory-scope`、`evolution-lessons-skills`、`evolution-session-search`。前两者承载 §53 的记录条件而非固定文本：每行点名触发的条件、其计数，以及可据以行动的界面；当贡献没有任何条件成立时，它什么也不渲染。技能贡献仅在可见的 `skill_manage` 工具旁渲染，因为其中一行要求模型用该工具记录经验。由于记忆与技能提示是按回合触发的，它们注册为动态的 `systemPrompt.context()` 条目而非静态分节（S1）：触发的条件以一条紧靠历史尾部、紧邻其触发回合的新鲜运行时上下文快照送达模型，而不是永久烘焙进系统提示头部的一行文字。会话搜索提示是一条常驻通知，不是条件，因此仍以静态 `systemPrompt.section()` 注册——但仅在真正挂载了会话搜索 seam（`ctx.sessionQuery.searchSessions`）时才渲染其文本；未挂载时该分节仍会注册，但渲染为空（S1）。
