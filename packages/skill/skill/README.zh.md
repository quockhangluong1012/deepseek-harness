...
- **加载期声明原样传递。** 加载的定义可能携带其所需的 `requiredEnv` 名称、其自身默认值的 `config` 映射，以及一份安装 `blueprint`（`{ schedule, deliver, prompt }`）。注册表校验这三者并把它们交给加载方消费者（面向模型的加载器负责解析前两者）；形态错误的 blueprint 会被丢弃，而不会令加载失败。摘要保持与调用无关，因此它们都不会进入目录，而 blueprint 始终只是建议——没有任何东西会自行排程。例外是与路由相关的关系：校验后的 `requires`、`conflicts_with` 与 `capabilities` 列表会随摘要提供给选择器，组合性声明同样如此——`compatible_with` 与 `composable_with`（分别由加载器与合成强制的白名单）、`inputs` 与 `outputs`（数据名称，只携带、从不门控），以及 `derived_from`（正文由哪些 skill 合成而来）——其它声明仍只停留在加载期。
...
提供方观测可以为其跳过的 skill 报告 `quarantinedCount`，例如未通过安全扫描的项目 skill。注册表会累加最近一次完成的发现所见的计数，并在 `skills/change` 事件上报告该总数；该计数属于宿主日志证据，绝不进入面向模型的目录。
...
| [`src/rank.ts`](src/rank.ts) | 按查询排序：在路由文本上做 BM25 粗排，再按嵌入相似度与下游效用重排 |
| [`src/composition.ts`](src/composition.ts) | 已声明成对关系规则：加载集合是否自洽、合成来源集合是否可合成，共用同一个 `CompositionMember` 形状 |
...
注册表没有 TTL：只有提供方调用其注册作用域内的 `invalidate()`，或发生运行时注册或释放时，才会清除已完成的目录。每次失效都会递增 revision、清空缓存，并发出 `skills/change` 事件，其负载携带最近一次完成的发现所观察到的隔离 skill 计数（尚未完成任何一次发现时为零）；消费方用各自的查找选项重新获取。`invalidate()` 仅当接收它的那条精确注册仍处于活动状态时才生效，因此延迟回调无法干扰同名替代提供方。

### 按查询排序

`rankSkills(query, skills, nameOf, textOf, options?)` 为单个查询排序候选项而不触碰注册表：在名称、描述与 `whenToUse` 上做 BM25 粗排；调用方提供向量时按嵌入余弦相似度重排，提供信号（`SkillRankSignal`：信任状态加已记录失败率，由调用方从遥测映射，无需包依赖）时按下游效用重排。调用方还可提供每个 skill 声明的关系，均来自 frontmatter：前置（`requires`）、skill 提供的能力（`capabilities`），以及不得与之同选的对手（`conflicts`）。前置既可由同名候选项满足，也可由以能力提供它的候选项满足；候选集无法满足其前置的 skill 得零分，因为路由到一个缺了兄弟就无法工作的 skill 永远不是正确的选择。冲突双方中排名较高者保留得分，另一方得零分；不可路由的候选项永不排除兄弟项。排序是纯同步函数——无 IO、无模型调用——因此每回合的目录路径不受影响；今天它唯一的消费方是行为评估的路由门，而这正是该排序为之构建的离线选择器。不存在会变质的排序缓存：排序按调用方提供的修订键逐次计算，而注册表按 revision 键控的收集中缓存本来就会在技能演进时失效。
...
- **排序不重排目录**——`rankSkills` 是纯离线选择器；每回合目录保持静态优先级顺序，直到一次可度量的检索失败证明应当把排序接入面向模型的路径。
- **组合只在选择集合时生效**——`requires` 与 `conflicts_with` 由离线排序器、行为路由门与加载器的已声明组合强制执行，而 `compatible_with` 只约束加载，因为排序器一次只评判一个候选项；分别加载的两个冲突 skill 不会被拒绝，因为两次调用互不可见。`composable_with` 同样只约束指名其来源的那次合成。
