...
- **加载期声明原样传递。** 加载的定义可能携带其所需的 `requiredEnv` 名称、其自身默认值的 `config` 映射，以及一份安装 `blueprint`（`{ schedule, deliver, prompt }`）。注册表校验这三者并把它们交给加载方消费者（面向模型的加载器负责解析前两者）；形态错误的 blueprint 会被丢弃，而不会令加载失败。摘要保持与调用无关，因此它们都不会进入目录，而 blueprint 始终只是建议——没有任何东西会自行排程。例外是与路由相关的关系：校验后的 `requires`、`conflicts_with` 与 `capabilities` 列表会随摘要提供给选择器，其它声明仍只停留在加载期。
...
- **组合只在选择集合时生效**——`requires` 与 `conflicts_with` 由离线排序器、行为路由门与加载器的已声明组合强制执行；分别加载的两个冲突 skill 不会被拒绝，因为两次调用互不可见。
