...
- **加载期声明原样传递。** 加载的定义可能携带其所需的 `requiredEnv` 名称、其自身默认值的 `config` 映射，以及一份安装 `blueprint`（`{ schedule, deliver, prompt }`）。注册表校验这三者并把它们交给加载方消费者（面向模型的加载器负责解析前两者）；形态错误的 blueprint 会被丢弃，而不会令加载失败。摘要保持与调用无关，因此它们都不会进入目录，而 blueprint 始终只是建议——没有任何东西会自行排程。唯一的例外是 `requires`：前置 skill 名称与路由相关，因此校验后的 `requires` 列表会随摘要提供给选择器，其它声明仍只停留在加载期。
...
`rankSkills(query, skills, nameOf, textOf, options?)` 为单个查询排序候选项而不触碰注册表：在名称、描述与 `whenToUse` 上做 BM25 粗排；调用方提供向量时按嵌入余弦相似度重排，提供信号（`SkillRankSignal`：信任状态加已记录失败率，由调用方从遥测映射，无需包依赖）时按下游效用重排。调用方还可按 skill 提供前置名称（`requires`，来自 frontmatter）：候选集中缺失任一前置的 skill 得零分，因为路由到一个缺了兄弟就无法工作的 skill 永远不是正确的选择。排序是纯同步函数——无 IO、无模型调用——因此每回合的目录路径不受影响；今天它唯一的消费方是行为评估的路由门，而这正是该排序为之构建的离线选择器。不存在会变质的排序缓存：排序按调用方提供的修订键逐次计算，而注册表按 revision 键控的收集中缓存本来就会在技能演进时失效。
