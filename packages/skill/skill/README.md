...
- **Composition binds only where a set is selected** — `requires` and `conflicts_with` are enforced by the offline ranker, the behavior routing gate, and the loader's declared composition; two conflicting skills loaded in separate calls are not refused, because neither call sees the other.
