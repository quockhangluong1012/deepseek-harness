...
- **已声明组合。** 一次加载返回所请求的 skill，外加它声明的每个前置，每个都渲染为紧随其后、按声明顺序排列的独立 `<skill_content>` 块；结果通过 `composed` 携带它们。既无同名目录 skill 也无提供该能力者可供解析的前置会令整次加载失败，报 `Error: skill "<name>" requires "<prerequisite>", which is not available in this session`；成员声明了冲突的集合同样失败，报 `Error: skill "<name>" cannot load: "<a>" and "<b>" declare a conflict`；成员落在 `compatible_with` 白名单之外的集合也会失败，报 `Error: skill "<name>" cannot load: "<a>" is not compatible with "<b>"`——半套已声明组合永不加载。
