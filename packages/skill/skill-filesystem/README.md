...
`requires` is parsed but never gates: a skill that names prerequisite skills is still offered, loaded, and listed, with the names carried on its summary and definition. Enforcement belongs to selectors — the offline ranker scores a skill zero when its prerequisites are absent from the candidate set — because only the selector knows which siblings are actually routable in that call.
