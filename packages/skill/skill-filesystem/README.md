...
A skill is either a directory bundle `<name>/SKILL.md` or a flat file `<name>.md` at the top level of a scanned root; nested `**/SKILL.md` files are deliberately not discovered. The file starts with YAML frontmatter: required `name` and `description`, plus optional `whenToUse`, `metadata`, `disable-model-invocation`, `user-invocable`, `required_env` (a non-empty list of environment variable names the skill needs), `requires` (a non-empty list of prerequisite skill names that must route alongside it), `conflicts_with` (a non-empty list of skill names this skill must not be selected alongside; one side declaring it excludes the pair), `capabilities` (a non-empty list of capability names that satisfy another skill's `requires`), `compatible_with` (a non-empty list of skills this one may load beside; declaring it makes the list an allowlist), `composable_with` (a non-empty list of skills this one may be synthesized with by `skill_manage derive`; also an allowlist), `inputs` and `outputs` (non-empty lists of data names the skill consumes and produces, carried for composition like `capabilities`), `config` (a mapping whose scalar values are stored as their string forms), and the [gating keys](#platform-and-tool-gating) below. Every other top-level key is ignored with one warning naming the file and the key, and a recognized key with a malformed value — an empty `required_env`, a non-string entry, a `config` that is not a mapping, a `config` value that is not a scalar, an empty list where a non-empty one is required, or a `blueprint` missing its schedule, deliver mode, or prompt — is dropped with one warning naming the file and the key; the skill still loads either way.
...
`requires` and `conflicts_with` are parsed but never gate: a skill declaring either relation is still offered, loaded, and listed, with the names carried on its summary and definition. A prerequisite is satisfied by a candidate carrying that name or by one that provides it as a capability. Enforcement belongs to selection — the offline ranker scores a skill zero when its prerequisites are absent from the candidate set and zeroes the lower-ranked side of a conflicting pair, and the loader refuses a load set containing a declared conflict — because only the selector knows which siblings are actually routable in that call.

`compatible_with` and `composable_with` are enforced at the two points that assemble a set, both through the same pair rule the conflict refusal uses. A non-empty `compatible_with` on any member of a load set refuses the load naming the incompatible pair, on top of the conflict refusal; a non-empty `composable_with` on any source of a `skill_manage derive` refuses that synthesis the same way. An absent or empty list constrains nobody, and a declaration naming a skill outside the set in front of it is ignored — neither the loader nor the derivation owns skills it was not asked about. `inputs` and `outputs` are carried but gate nothing, like `capabilities`.
