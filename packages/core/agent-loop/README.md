...
The generated [configuration catalog](../../../docs/config-catalog.md#deepseek-aidsh-agent-loop) lists every accepted field. The adapter validates the effective reasoning effort and the loop records it in the request header. `maxParallelToolCalls`, `maxSteps`, and `maxRequestRetries` are volatile Config fields sampled for the next tool group, turn continuation, or retry decision, so a profile edit applies without a restart; `agents` remains startup configuration. `maxSteps` ends a turn that would enter more steps than the ceiling with `max-steps` instead of running unbounded, even when a steer queued more work, and `maxRequestRetries` caps honored request recoveries per step, so a listener that recovers unconditionally cannot retry forever; provider retry policies stay authoritative below the ceiling.
...
### Injected context and the surface

A pre-step listener returns the messages it wants committed, and the loop appends each one to the session log. When a message's source declares `form: 'snapshot'` **and** `supersedes`, the loop instead appends it as a surface replacement of that producer's previous snapshot: the producer, not the message, is the slot, so a newer brief with different sections, digest, or scope still supersedes the older one. The log keeps every injection — a transcript and a log replay see each snapshot where it was committed — while `deriveMessages()` and every request read the surface, so the model reads the live message once.

A snapshot that does not declare `supersedes` accumulates, which is what a producer wants when its entries are a timeline rather than a state: `time-context`'s later reading measures elapsed time against the earlier one, so both must stay.
...
Final adapter selection, dispatch, and iteration failures arrive as terminal finishes and enter `agent/request-error`; a handling listener returns `{ kind: 'retry' }` without calling `next()`, while an unhandled failure is terminal. Honored retries per step are capped by `maxRequestRetries`; a recovery past the ceiling stays terminal and warns, so unconditional recovery cannot retry forever. Middleware, result-processing, tool, and other extension failures remain thrown and close the turn directly — plugin failure ends the turn, not the loop. Undispatched model tool calls after cancellation receive synthetic `tool/call` plus `ABORTED_BEFORE_DISPATCH` result pairs. A wake arriving inside an open turn is latched, and a turn that then dies inside a step — or under cancellation — replays the latch at convergence, so a prompt queued during a failing turn runs in a fresh turn instead of being stranded; a re-woken turn that fails again with no new wake ends the loop there rather than retrying itself. A blocked turn and a failing `agent/pre-step` hook still park pending work for an explicit wake, and `cancel()` drops the latch, so `keepInbox` parks pending work until a later wake. The [explicit-cancellation decision](../../../.agents/notes/implemented/architecture/2026-07-16-explicit-turn-cancellation.md) owns the signal lifecycle.
