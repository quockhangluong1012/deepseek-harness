...
生成的[配置目录](../../../docs/config-catalog.zh.md#deepseek-aidsh-agent-loop) 列出所有接受的字段。适配器会校验有效推理强度，循环则把它记录在请求头中。`maxParallelToolCalls`、`maxSteps` 与 `maxRequestRetries` 是 volatile Config 字段，分别在下一个工具组、下一次轮次续行、下一次重试决策时读取，因此修改 profile 无需重启即可生效；`agents` 仍是启动配置。`maxSteps` 将超出上限的 turn 以 `max-steps` 结束而不是无界运行，即使某次 steer 排入了更多工作；`maxRequestRetries` 限制每个步骤接受的请求恢复次数，无条件恢复的监听器因此无法无限重试，提供方重试策略在上限之下保持权威。
