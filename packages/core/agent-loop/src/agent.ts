  ...
  private async turn(): Promise<boolean> {
    ...
    try {
      while (true) {
        ...
        if (phase.step >= this.loopCtx.agentLoop.config.maxSteps.get() && !isSettledCeiling(turnEnds)) {
          stepCeilingHit = true
          break
        }
        target = 'next-step'
  ...
  private async step(decision: Extract<PreparedStep, { kind: 'enter' }>): Promise<StepEndReason | null> {
    ...
    while (true) {
      ...
      const commits = this.systemPrompt.project(renderedPrompt, {
        ...
        startsSeries: this.seriesFromSurface(startsRequestSeries, this.session.surface.contentGeneration)
      ...
      try {
        ...
        if (finish.kind === 'error' || finish.kind === 'aborted') {
          ...
          requestRetries += 1
          if (requestRetries > this.loopCtx.agentLoop.config.maxRequestRetries.get()) {
