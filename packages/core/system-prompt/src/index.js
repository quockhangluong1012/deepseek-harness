...
function interpolate(input, variables, kind) {
    ...
    for (let open = text.indexOf('{{'); open >= 0; open = text.indexOf('{{', last)) {
        ...
        let backslashes = 0;
        for (let i = open - 1; i >= last && text[i] === '\\'; i--)
            backslashes += 1;
        if (backslashes % 2 === 1) {
 ...
 */
function validateStaticTemplate(name, text, kind) {
    const acceptAll = new Proxy({}, {
...
export class SystemPrompt extends Service {
    ...
    section(section) {
        ...
        if (typeof section.text === 'string')
            validateStaticTemplate(section.name, section.text, 'section');
    ...
    context(context) {
        ...
        if (typeof context.text === 'string')
            validateStaticTemplate(context.name, context.text, 'context');
