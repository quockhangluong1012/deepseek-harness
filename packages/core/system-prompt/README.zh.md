...
变量在段文本中以 `{{name}}` 引用，并在每次组装时解析；带作用域变量会为该 agent 遮蔽同名全局变量。循环提供 `model` 与 `cwd`；任何插件都可以注册自己拥有的事实。需要原样到达模型的字面量花括号请写成 `\{{`。静态段与上下文文本中格式错误的 `{{…}}` 组会在注册时被拒绝；未知变量名在组装时（此时已注册集合完整已知）被拒绝。
