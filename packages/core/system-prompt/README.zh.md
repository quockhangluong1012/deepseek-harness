...
变量在段文本中以 `{{name}}` 引用，并在每次组装时解析；带作用域变量会为该 agent 遮蔽同名全局变量。循环提供 `model` 与 `cwd`；任何插件都可以注册自己拥有的事实。需要原样到达模型的字面量花括号请写成 `\{{`。静态段与上下文文本中格式错误的 `{{…}}` 组会在注册时被拒绝——设置 `interpolate: false` 的段是字面量，不受该检查；未知变量名在组装时（此时已注册集合完整已知）被拒绝。
...
- **转义以组为单位，而非以段为单位**：`\{{` 只转义插值文本中的一个花括号对，因此整段（包括生成的工具文档）需要保持字面量时应设置 `interpolate: false`。
