...
`rotate` 让你不必先停用正在使用的密钥就能替换它：`await ctx.credentials.rotate(ref, current => nextKey(current))` 在写入所用的同一把写锁下，读取该引用当前解析出的值——已存储的值，或未存储时来自 `.env` 后备层的值——交给你的决定函数，并存储函数返回的值。存储的结果会压过它所替换的 `.env` 后备值，因此下一个按名引用该密钥的请求会使用它。
...
- **启动环境提供的密钥是只读的**——`DEEPSEEK_API_KEY=… dsh` 在本轮运行中优先，保存、移除或轮换它都会被拒绝。请先在启动 shell 中清除该变量。
...
`rotate` 走同一条链与同一把锁：它重新读取文档，把该引用当前解析出的值——已存储值或 `.env` 后备值——交给调用方的决定函数，在渲染前拒绝空结果，并提交返回的值、恰好触发一次 `credentials/reference-updated`。启动环境提供的引用会像 `set` 一样在入口被拒绝，因为一次会被解析持续忽略的已提交轮换，比一次失败的轮换更糟。
