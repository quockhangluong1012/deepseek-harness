---
description: "凭据能力族的包映射：凭据引用 seam、环境与文件提供方、授权 flow 注册表及其 Remote owner，以及引用如何让机密值留在配置之外。"
kind: "package-group"
---
...
四个包共同提供凭据功能：一个在运行时存储、查询与移除机密，而配置只写名字；第二个是默认的本机存储；第三个让插件获取必须向人请求的凭据；第四个通过 Remote 把该 flow 注册表提供给浏览器界面。它们的 README 覆盖日常使用；全部约定以子系统参考为准。
