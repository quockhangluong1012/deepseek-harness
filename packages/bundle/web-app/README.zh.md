...
运行 `dsh --profile web`，打开提供聊天、模型与设置管理以及会话历史的交互式浏览器 GUI。它使用与其他 dsh 表层相同的模型访问、工具与安全默认值。Web 组合还会以 shadow 模式挂载上下文编译器，记录来源放置而不更改组装后的请求。启动时会打印带认证信息的 URL，通常还会在默认浏览器中打开；SSH 会话和 `--no-open` 会保留该 URL，供你手动打开。你可以更改端口并允许额外主机，但不能绑定所有网络接口。需要在浏览器中交互式工作时选择本包；一次性的命令行任务应使用 `dsh-headless`。
...
- [dsh-agent-context](../../runtime/agent-context/README.zh.md)——来源放置与预算化提示组装。
...
编译器会评估每次 Web 组装，并为每个不同的放置结果记录一条仅写入日志的 `context/compiled` 事件。`shadow` 模式会原样返回组装后的请求。
...
- **上下文放置仅供观察**——Web 编译器以 `shadow` 模式运行且没有 token 上限，因此只记录放置结果，不会丢弃来源。改为 `apply` 需要由部署明确设置上限。
