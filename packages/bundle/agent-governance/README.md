...
Add this layer to a profile that must record what each session decided and, when the deployment is ready, act on it. It inserts the agent kernel in shadow mode, the built-in declarations that let the kernel evaluate every shipped tool, and a prompt-injection guard that records what untrusted content tried to do. No shipped profile includes it: a deployment opts in, measures a permission document against real traffic, then flips `mode` to enforce. Everything it writes is log-only.
...
The profile resolves the layer from the installed package and reconciles the three rows below into its composition; removing it drops those rows. If a row's package is missing from the profile, that row fails to resolve and the profile reports the unresolved entry rather than starting without it.

### What you get

Three rows, each owned by the package it names. `agent-kernel` records one durable task contract per session, proposes and decides every tool action against its permission document, and runs the completion gate — logging `task/*`, `action/*`, `policy/decision`, `verification/*`, and `failure/recorded` events. `agent-kernel-builtins` declares what each shipped product tool needs, so a shadow kernel's record names the real capability instead of a blanket denial. `prompt-injection` wraps tool results and model proposals in a tainted envelope, records `security/scan` when a known rule matches, and, in `enforce`, replaces credentials in the model-visible copy.
...
Every row starts in `shadow`. A shadow kernel records the decision it would have made and lets execution proceed, and a shadow guard records findings without rewriting content, so adding this layer to a running profile changes logs — never behavior — until an operator edits the row.

| File | Role |
|---|---|
| [`cordis.patch.yml`](cordis.patch.yml) | The three rows, their `shadow` defaults, and the reason each one is separate |
...
- **It inserts no configuration beyond the modes** — the permission document, acceptance criteria, and scan ceiling belong to the deployment's own profile patch, not to this layer.
