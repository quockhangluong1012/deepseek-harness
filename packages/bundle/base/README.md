...
- **Hook config paths are launch-cwd relative and read once** — `hooks-claude-code`/`hooks-codex` resolve `./.claude/settings.json` and `./.codex/hooks.json` against the process launch directory at startup; a project opened later, or a project switch mid-process, does not pick up a different file. A missing file is not an error: the bridge logs a warning and runs no hooks.
