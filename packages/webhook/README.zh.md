...
Webhook 系列接收通过身份验证的提供方事件，并运行受信任的程序化规则。规则可以在 Web Workspace 中创建普通根会话。分发仅存在于进程内并采用 fire-and-forget，不拥有交付数据库、队列、重试或 agent（智能体）完成状态——只有一个有界一小时重放窗口会丢弃重复投递的 id。
