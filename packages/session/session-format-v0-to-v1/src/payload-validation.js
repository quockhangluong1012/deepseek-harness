 ...
 */
export function assertReleasedPayloadSemantics(event, version) {
    ...
    switch (event.type) {
        ...
        case 'approval/policy':
            ...
            if (data['source'] !== undefined)
                literalValue(data['source'], ['delegation'], `${label} source`);
            return;
        ...
        case 'assistant/message':
            ...
            if (data['usage'] !== undefined)
                tokenUsageValue(data['usage'], `${label} usage`);
            if (data['interrupted'] !== undefined)
                literalValue(data['interrupted'], [true], `${label} interrupted`);
            return;
        case 'command/done':
            nonEmptyString(data['commandId'], `${label} commandId`);
            literalValue(data['kind'], ['success', 'error'], `${label} kind`);
            if (data['text'] !== undefined)
                stringValue(data['text'], `${label} text`);
            if (data['sourceEventSeq'] !== undefined)
                earlierSeq(data['sourceEventSeq'], event.seq, `${label} sourceEventSeq`);
            return;
        case 'command/run': {
            nonEmptyString(data['commandId'], `${label} commandId`);
            nonEmptyString(data['name'], `${label} name`);
            if (data['args'] !== undefined)
                stringValue(data['args'], `${label} args`);
        ...
        case 'compaction/end':
            ...
            if (data['sourceCommandId'] !== undefined)
                nonEmptyString(data['sourceCommandId'], `${label} sourceCommandId`);
        ...
        case 'compaction/summary':
            ...
            if (data['maxTokens'] !== undefined)
                countValue(data['maxTokens'], `${label} maxTokens`);
            if (data['usage'] !== undefined)
                tokenUsageValue(data['usage'], `${label} usage`);
            if (data['rawOutput'] !== undefined)
                contentBlocksValue(data['rawOutput'], `${label} rawOutput`, version);
            if (data['llmStreamCall'] !== undefined)
                literalValue(data['llmStreamCall'], [true], `${label} llmStreamCall`);
            return;
        ...
        case 'hook/invoked':
            ...
            if (data['matcher'] !== undefined)
                stringValue(data['matcher'], `${label} matcher`);
        ...
        case 'hook/result':
            ...
            if (data['exitCode'] !== undefined)
                safeIntegerValue(data['exitCode'], `${label} exitCode`);
            if (data['stderrSummary'] !== undefined)
                stringValue(data['stderrSummary'], `${label} stderrSummary`);
        ...
        case 'llm/retry':
            ...
            if (data['mode'] === 'normal') {
                ...
                if (data['retry'] > maxRetries)
                    throw new SessionFormatError(`${label} retry exceeds maxRetries`);
            }
            ...
            if (delayMs < 0)
                throw new SessionFormatError(`${label} delayMs must be non-negative`);
        ...
        case 'model/selection':
            ...
            if (data['reasoningEffort'] !== undefined)
                nonEmptyString(data['reasoningEffort'], `${label} reasoningEffort`);
            return;
        ...
        case 'request/context':
            ...
            if (data['contextWindow'] !== undefined)
                positiveIntegerValue(data['contextWindow'], `${label} contextWindow`);
            return;
        case 'request/header':
            requestHeaderValue(data['header'], `${label} header`);
            literalValue(data['reason'], ['initial', 'resume', 'change', 'series'], `${label} reason`);
            if (data['startsSeries'] !== undefined)
                literalValue(data['startsSeries'], [true], `${label} startsSeries`);
            return;
        case 'sandbox/mode':
        ...
        case 'tool-workflow/agent-start':
            ...
            if (data['phase'] !== undefined)
                stringValue(data['phase'], `${label} phase`);
...
function stringValue(value, label) {
    if (typeof value !== 'string')
        throw new SessionFormatError(`${label} must be a string`);
}
function nonEmptyString(value, label) {
    if (typeof value !== 'string' || value.length === 0)
        throw new SessionFormatError(`${label} must be a non-empty string`);
}
function booleanValue(value, label) {
    if (typeof value !== 'boolean')
        throw new SessionFormatError(`${label} must be a boolean`);
}
...
function positiveIntegerValue(value, label) {
    ...
    if (result === 0)
        throw new SessionFormatError(`${label} must be positive`);
    return result;
}
...
function arrayValue(value, label, validate) {
    if (!Array.isArray(value))
        throw new SessionFormatError(`${label} must be an array`);
    const members = value;
...
function earlierSeq(value, eventSeq, label) {
    ...
    if (seq >= eventSeq)
        throw new SessionFormatError(`${label} must identify an earlier event`);
    return seq;
}
function seqArray(value, eventSeq, label, requireNonEmpty) {
    ...
    const values = arrayValue(value, label, (member, memberLabel) => {
        ...
        if (seen.has(seq))
            throw new SessionFormatError(`${label} repeats seq ${seq}`);
        seen.add(seq);
    });
    if (requireNonEmpty && values.length === 0)
        throw new SessionFormatError(`${label} must be non-empty`);
    return values;
}
function llmFailureValue(value, label) {
    ...
    if (failure['status'] !== undefined) {
        ...
        if (status < 100 || status > 599)
            throw new SessionFormatError(`${label} status must be 100 through 599`);
    }
    ...
    if (failure['requestId'] !== undefined)
        nonEmptyString(failure['requestId'], `${label} requestId`);
}
function tokenUsageValue(value, label) {
    const usage = exactRecord(value, label, ['inputTokens', 'outputTokens'], ['totalTokens', 'cacheReadTokens', 'cacheWriteTokens', 'reasoningTokens']);
    ...
    switch (block['type']) {
        ...
        case 'tool-result':
            ...
            if (block['isError'] !== undefined)
                booleanValue(block['isError'], `${label} isError`);
            return;
        default:
...
function imageAttachmentValue(value, label) {
    const attachment = exactRecord(value, label, ['attachmentId', 'mediaType', 'bytes', 'width', 'height'], ['name', 'originalDimensions']);
...
function messageValue(value, label, version, expected) {
    ...
    if (role === undefined)
        literalValue(message['role'], ['system', 'user', 'assistant'], `${label} role`);
    else
        literalValue(message['role'], [role], `${label} role`);
...
function legacyGoalMessageValue(message, source, label) {
    ...
    if (source['round'] !== 0)
        throw new SessionFormatError(`${label} legacy goal source round must be 0`);
...
function messageSourceValue(value, label, version, expected) {
    ...
    if (expected === 'assistant' && source['kind'] !== 'model')
        throw new SessionFormatError(`${label} must be model source`);
    if (expected === 'tool' && source['kind'] !== 'tool')
        throw new SessionFormatError(`${label} must be tool source`);
    switch (source['kind']) {
        case 'user':
            assertReleasedV0Keys(source, ['kind'], ['rpcId', 'clientTimeZone'], label);
            if (source['rpcId'] !== undefined)
                nonEmptyString(source['rpcId'], `${label} rpcId`);
            if (source['clientTimeZone'] !== undefined)
                nonEmptyString(source['clientTimeZone'], `${label} clientTimeZone`);
            return;
        case 'plugin':
        ...
        case 'agent-instructions':
            ...
            if (source['baseline'] !== undefined)
                literalValue(source['baseline'], [true], `${label} baseline`);
            if (source['baselineIdentity'] !== undefined)
                nonEmptyString(source['baselineIdentity'], `${label} baselineIdentity`);
            arrayValue(source['changes'], `${label} changes`, (member, memberLabel) => {
                ...
                if (change['digest'] !== undefined)
                    stringValue(change['digest'], `${memberLabel} digest`);
            });
            return;
        ...
        case 'team-message':
            ...
            for (const key of ['teamId', 'messageId', 'senderId'])
        ...
        case 'skill-catalog':
            ...
            if (source['update'] !== undefined)
                literalValue(source['update'], [true], `${label} update`);
        ...
        case 'webhook':
            ...
            for (const key of ['provider', 'source', 'deliveryId', 'ruleId'])
                nonEmptyString(source[key], `${label} ${key}`);
            literalValue(source['form'], ['notice'], `${label} form`);
            stringValue(source['summary'], `${label} summary`);
            return;
...
function pluginSourceValue(source, label) {
    ...
    if (source['plugin'] === 'compact')
        optional.push('compactionId', 'sourceCommandId');
    ...
    if (source['plugin'] === 'compact') {
        ...
        if (source['sourceCommandId'] !== undefined)
            nonEmptyString(source['sourceCommandId'], `${label} sourceCommandId`);
    }
...
function sessionReferenceSourceValue(source, label, version) {
    ...
    const references = arrayValue(source['references'], `${label} references`, (member, memberLabel) => {
        ...
        if (reference['capturedThroughSeq'] !== null)
            countValue(reference['capturedThroughSeq'], `${memberLabel} capturedThroughSeq`);
        ...
        if (inputIndex !== expectedInputIndex)
            throw new SessionFormatError(`${label} inputIndex must match reference position`);
        expectedInputIndex += 1;
        const sessionId = reference['sessionId'];
        if (sessionIds.has(sessionId))
            throw new SessionFormatError(`${label} repeats sessionId ${sessionId}`);
        sessionIds.add(sessionId);
    });
    if (references.length === 0)
        throw new SessionFormatError(`${label} references must be non-empty`);
}
    ...
    switch (chunk['type']) {
        ...
        case 'tool-call-delta':
            ...
            if (chunk['name'] !== undefined)
                stringValue(chunk['name'], `${label} name`);
        ...
        case 'finish':
            ...
            if (chunk['replayState'] !== undefined)
                replayEnvelopeValue(chunk['replayState'], `${label} replayState`);
            return;
        default:
...
function requestHeaderValue(value, label) {
    ...
    const config = exactRecord(header['config'], `${label} config`, ['provider', 'model'], ['reasoningEffort', 'temperature', 'maxTokens', 'stop']);
    nonEmptyString(config['provider'], `${label} provider`);
    nonEmptyString(config['model'], `${label} model`);
    if (config['reasoningEffort'] !== undefined)
        nonEmptyString(config['reasoningEffort'], `${label} reasoningEffort`);
    if (config['temperature'] !== undefined)
        finiteNumberValue(config['temperature'], `${label} temperature`);
    if (config['maxTokens'] !== undefined)
        positiveIntegerValue(config['maxTokens'], `${label} maxTokens`);
    if (config['stop'] !== undefined)
        arrayValue(config['stop'], `${label} stop`, stringValue);
    if (header['adapterDefaults'] !== undefined) {
        ...
        for (const [key, marker] of Object.entries(defaults)) {
            ...
            if (!Object.hasOwn(config, key))
                throw new SessionFormatError(`${label} adapter default ${key} lacks config value`);
        }
    }
    if (header['system'] !== undefined)
        stringValue(header['system'], `${label} system`);
    if (header['tools'] !== undefined)
        arrayValue(header['tools'], `${label} tools`, toolSchemaValue);
}
...
function goalChangeValue(data, label) {
    ...
    assertReleasedV0Keys(data, ['kind', 'version', 'operation', 'goal', 'roundsStarted', 'createdAt', 'updatedAt'], [], `${label} data`);
...
function scheduleChangeValue(data, label) {
    ...
    assertReleasedV0Keys(data, ['version', 'operation', 'id'], data['operation'] === 'dispatch' ? ['acceptedAt'] : [], `${label} data`);
    literalValue(data['operation'], ['delete', 'dispatch'], `${label} operation`);
    scheduleIdValue(data['id'], `${label} id`);
    if (data['acceptedAt'] !== undefined)
        instantValue(data['acceptedAt'], `${label} acceptedAt`);
}
function scheduleRecordValue(value, label) {
    ...
    if (record['kind'] === 'after') {
        ...
        if (seconds < 300)
            throw new SessionFormatError(`${label} everySeconds must be at least 300`);
    }
    else {
...
function scheduleIdValue(value, label) {
    ...
    if (value.trim() !== value)
        throw new SessionFormatError(`${label} must not have surrounding whitespace`);
}
...
function titleSourceValue(value, label) {
    ...
    if (source['kind'] === 'provider') {
        ...
        if (source['model'] !== undefined)
            modelRouteValue(source['model'], `${label} model`);
        return;
    }
...
function subagentDescriptorValue(data, label) {
    ...
    for (const key of ['agentProvider', 'agentModel', 'agentReasoningEffort', 'persona']) {
        if (data[key] !== undefined)
            nonEmptyString(data[key], `${label} ${key}`);
    }
    ...
    if (data['toolFilter'] !== undefined) {
        ...
        if (filter['allow'] !== undefined)
            arrayValue(filter['allow'], `${label} allow`, nonEmptyString);
        if (filter['deny'] !== undefined)
            arrayValue(filter['deny'], `${label} deny`, nonEmptyString);
    }
}
function allowedModelsValue(value, label) {
    ...
    const routes = arrayValue(value, label, (member, memberLabel) => {
        ...
        if (seen.has(key))
            throw new SessionFormatError(`${label} repeats route ${key}`);
        seen.add(key);
    });
    if (routes.length === 0)
        throw new SessionFormatError(`${label} must be non-empty`);
...
function teamMemberValue(value, label) {
    ...
    if (member['error'] !== undefined)
        stringValue(member['error'], `${label} error`);
}
function teamTaskValue(value, label) {
    const task = exactRecord(value, label, ['id', 'revision', 'subject', 'description', 'status', 'blockedBy', 'writeScopes'], ['ownerId']);
    ...
    if (task['ownerId'] !== undefined)
        nonEmptyString(task['ownerId'], `${label} ownerId`);
...
function teamMessageValue(value, label, version) {
    ...
    for (const key of ['id', 'senderId', 'targetId'])
...
function deepSeekSearchBodyValue(value, label) {
    ...
    const messages = arrayValue(body['messages'], `${label} messages`, (member, memberLabel) => {
        ...
        if (content.length !== 1)
            throw new SessionFormatError(`${memberLabel} content must contain one text block`);
    });
    if (messages.length !== 1)
        throw new SessionFormatError(`${label} messages must contain one user message`);
    ...
    if (tools.length !== 1)
        throw new SessionFormatError(`${label} tools must contain one web search tool`);
}
