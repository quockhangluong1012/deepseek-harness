 ...
 * @param label - diagnostic subject.
 */
export function keys(value, required, optional, label) {
    const missing = required.find(key => !Object.hasOwn(value, key));
    const unexpected = Object.keys(value).find(key => !required.includes(key) && !optional.includes(key));
    if (missing !== undefined)
        throw new SessionFormatError(label + ' lacks required field ' + missing);
 ...
 */
export function assertEvent(event, version) {
    if (version === 3) {
        assertV3Event(event);
        return;
    }
    ...
    if (surface) {
        ...
        if (event['surfaceOp'] === undefined)
            throw new SessionFormatError(event.type + ' requires surfaceOp');
    }
    ...
    if (event.type === 'assistant/message' || event.type === 'assistant/attempt') {
        for (const coordinate of ['turn', 'step']) {
            if (sessionFormatCount(data[coordinate], coordinate) === 0)
                throw new SessionFormatError(coordinate + ' must be positive');
        }
    }
    ...
    if (event.type === 'assistant/message' || event.type === 'tool/result')
        assertSource(record(data['message'], 'message'));
 ...
 */
export function isRepairIdentity(id, callId) {
...
function contentArray(value, label) {
    if (!Array.isArray(value))
        throw new SessionFormatError(label + ': content must be an array');
    return value;
}
    ...
    switch (event.type) {
        ...
        case 'compaction/summary':
            ...
            if (data['rawOutput'] !== undefined)
                assertContentKinds(data['rawOutput'], label + '.rawOutput');
            break;
    }
    if (event.type === 'assistant/message' || event.type === 'assistant/attempt') {
        for (const [index, value] of contentArray(data['stream'], label + '.stream').entries()) {
            ...
            if (chunk['type'] === 'block-end')
                assertContentBlock(chunk['block'], path + '.chunk.block');
            if (chunk['type'] === 'block-start')
                assertContentKind(chunk['blockType'], path + '.chunk.blockType');
        }
    }
}
...
function assertContentKinds(content, label) {
    for (const [index, value] of contentArray(content, label).entries())
        assertContentBlock(value, label + '[' + String(index) + ']');
}
function assertContentBlock(value, label) {
    ...
    if (block['type'] === 'tool-result') {
        if (!Array.isArray(block['content']))
            throw new SessionFormatError(label + '.content: invalid message content kind "tool-result": content must be an array');
    ...
    if (block['type'] === 'file') {
        ...
        if (typeof attachment['attachmentId'] !== 'string' || attachment['attachmentId'].length === 0
            ...
            throw new SessionFormatError(label + ' kind "file": file attachment requires attachmentId and name');
    ...
    const probe = { type: 'user/message', seq: 0, time: 0, data: {
 ...
 */
export function assertV3StructuralRow(value) {
...
function assertFeedback(type, data) {
    ...
    if (typeof data['sessionId'] !== 'string')
        throw new SessionFormatError('feedback sessionId must be a string');
    if (type === 'feedback/message-delete') {
        if (typeof data['messageId'] !== 'string')
            throw new SessionFormatError('feedback messageId must be a string');
        return;
    }
    const item = record(data['item'], 'feedback item');
    keys(item, ['messageId', 'rating', 'version', 'createdAt', 'updatedAt'], ['note'], 'feedback item');
    for (const key of ['messageId', 'version'])
        if (typeof item[key] !== 'string')
            throw new SessionFormatError('feedback ' + key + ' must be a string');
    if (item['rating'] !== 'positive' && item['rating'] !== 'negative')
        throw new SessionFormatError('invalid feedback rating');
    if (item['note'] !== undefined && typeof item['note'] !== 'string')
        throw new SessionFormatError('feedback note must be a string');
 ...
 */
export function assertV3Event(event, knownEventTypes) {
    ...
    const opaque = !known;
    keys(value, ['type', 'seq', 'time', 'data'], SURFACE_TYPES.has(event.type) || opaque ? ['ignorable', 'surfaceOp', 'sourceEventSeqs'] : ['ignorable'], subject);
    if (typeof event.type !== 'string')
        throw new SessionFormatError(`${subject} type must be a string`);
    ...
    if (SURFACE_TYPES.has(event.type)) {
        ...
        if (operation === undefined)
            throw new SessionFormatError(`${subject} requires a surfaceOp marker`);
        if (operation !== 'append') {
        ...
        if (sources !== undefined) {
            ...
            for (const source of sources) {
                ...
                if (seq >= event.seq || seen.has(seq))
                    throw new SessionFormatError(`${subject} sourceEventSeqs must be unique earlier seqs`);
                seen.add(seq);
            }
        }
    }
 ...
 */
export function canonicalizeTransformedEvent(event) {
    let target = event;
