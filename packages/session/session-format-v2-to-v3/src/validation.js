...
import { assertV3Event, isRepairIdentity, record, SURFACE_TYPES } from "./payload.js";
/**
 * Validate v3 logical metadata with the released-v2 fields.
 * @param header - decoded v3 Session header.
 */
export function assertReleasedV3Header(header) {
    if (header.version !== 3)
        throw new SessionFormatError('expected format v3 header');
 ...
 */
export function restoreReleasedV3Artifact(artifact, knownEventTypes) {
 ...
 */
export function assertV3EventAdmission(event) {
    if ((event.type === 'tool/code-dispatch-start' || event.type === 'tool/code-dispatch')
        && event['ignorable'] !== true) {
        throw new SessionFormatUnsupportedMigrationError('format v3 contains unknown event type ' + JSON.stringify(event.type) + ' at seq ' + String(event.seq));
    }
}
