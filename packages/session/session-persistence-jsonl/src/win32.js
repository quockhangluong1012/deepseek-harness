 ...
 */
export async function acquireLockHandleWin32(path) {
    const api = await win32();
    ...
    if (handle === 0)
        throw win32Error('CreateSemaphoreW', api.getLastError(), path, name);
    const wait = api.waitForSingleObject(handle, 0);
    if (wait === WAIT_OBJECT_0)
        return handle;
    api.closeHandle(handle);
    if (wait === WAIT_TIMEOUT)
        throw win32Error('WaitForSingleObject', ERROR_SHARING_VIOLATION, path, name);
 ...
 */
export async function releaseLockHandleWin32(handle) {
    const api = await win32();
    const released = api.releaseSemaphore(handle, 1, null);
    const closed = api.closeHandle(handle);
    if (released === 0 || closed === 0)
        throw win32Error('ReleaseSemaphore', api.getLastError(), `handle:${handle}`, `handle:${handle}`);
}
/**
