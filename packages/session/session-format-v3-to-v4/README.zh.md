...
- **V5 子日志证据**——V3 到 V4 的父日志准备仍通过已安装目录获取子事实。V4 到 V5 迁移边为自身的源转换固定使用 V4 编解码器和恢复器，但不会替换该子事实读取器。后续改变 V4 子日志语义的迁移边必须绑定固定的 V4 读取器。
- **历史嵌套工具结果**——当前迁移拒绝包含另一个 tool-result wrapper 的结果。原始代际保持完整，且不发布 V5 successor。后续转换器可以支持有证据的源数据场景，而不改变既定 V4 格式；[迁移 cookbook](../../../docs/cookbook/adding-a-session-format-version.zh.md#stages-and-validation) 定义了这一区别。
