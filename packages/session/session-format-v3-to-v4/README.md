...
- **V5 child evidence** — V3-to-V4 parent preparation still obtains child facts through the installed catalog. The V4-to-V5 edge pins the V4 codec and restorer for its own source conversion but does not replace that child-fact reader. A later edge that changes V4 child semantics must bind a fixed V4 reader.
- **Nested historical tool results** — migration currently refuses results containing another tool-result wrapper. The original generation remains intact and no V5 successor is published. A later converter may support evidenced source cases without changing the established V4 format; the [migration cookbook](../../../docs/cookbook/adding-a-session-format-version.md#stages-and-validation) defines that distinction.
