...
import { RELEASED_V2_EVENT_DISPOSITIONS } from "./dispositions.js";
 ...
 */
export function assertReleasedV2Header(header) {
    ...
    if (record['version'] !== 2)
        throw new SessionFormatError('expected format v2 header');
    if (typeof record['id'] !== 'string')
        throw new SessionFormatError('format v2 header id must be a string');
    ...
    if (typeof record['isSeeded'] !== 'boolean')
        throw new SessionFormatError('format v2 header isSeeded must be boolean');
...
function validateReleasedV2Artifact(artifact, mode, knownEventTypes, relationshipHeaderVersion = artifact.header.version) {
    ...
    if (cut > artifact.events.length)
        throw new SessionFormatError('format v2 inherited event count exceeds its events');
    if (!artifact.header.isSeeded && cut !== 0)
    ...
    for (const [index, event] of artifact.events.entries()) {
        ...
        if (typeof type !== 'string')
            throw new SessionFormatError(`format v2 event ${index} type must be a string`);
        ...
        if (mode === 'current' && disposition === undefined && !installed && !ignorableUnknown) {
            throw new SessionFormatUnsupportedMigrationError(`format v2 contains unknown event type ${JSON.stringify(type)} at seq ${index}`);
        }
        ...
        if (record['seq'] !== index)
            throw new SessionFormatError(`format v2 event ${index} is not dense`);
 ...
 */
export function releasedV2Record(value, label) {
 ...
 * @param label - diagnostic subject.
 */
export function assertReleasedV2Keys(value, required, optional, label) {
 ...
 */
export function restoreReleasedV2Artifact(artifact, knownEventTypes, relationshipHeaderVersion = artifact.header.version) {
 ...
 */
export function assertReleasedV2PhysicalArtifact(artifact) {
