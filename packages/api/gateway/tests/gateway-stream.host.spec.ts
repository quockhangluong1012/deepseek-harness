...
describe('Typert Remote streams', () => {
  ...
  it('validates the WebSocket heartbeat timer range and the stream inbox bound', () => {
    expect(TypertGatewayService.Config({}))
      .toEqual({ websocketHeartbeatIntervalMs: 2_000, websocketHeartbeatMaxMissed: 2, streamInboxBytes: 262_144 })
    expect(TypertGatewayService.Config({ websocketHeartbeatIntervalMs: MAX_TIMER_DELAY_MS, streamInboxBytes: 1 }))
      .toEqual({ websocketHeartbeatIntervalMs: MAX_TIMER_DELAY_MS, websocketHeartbeatMaxMissed: 2, streamInboxBytes: 1 })
    expect(TypertGatewayService.Config({ websocketHeartbeatMaxMissed: 1 }))
      .toEqual({ websocketHeartbeatIntervalMs: 2_000, websocketHeartbeatMaxMissed: 1, streamInboxBytes: 262_144 })
    ...
    for (const websocketHeartbeatMaxMissed of [0, 1.5]) {
      expect(() => TypertGatewayService.Config({ websocketHeartbeatMaxMissed })).toThrow()
    }
