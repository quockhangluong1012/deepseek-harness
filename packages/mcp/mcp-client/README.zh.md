...
SDK 通过旧版通知或现代协议订阅接收工具列表变化。监督器将每次重新同步排队；获取失败时保留之前的注册代，注册冲突则回滚本次尝试。每次故障共享一个尝试预算：连续失败达到 `maxAttempts` 后注销工具并停止重连；连接持续超过 `maxDelayMs` 则重置预算。每个已同步的世代还会在 `ctx.agentKernel` 存在时向其声明自己的工具——以 `mcp.call` 对应资源 `mcp:<serverName>/<rawToolName>`——并在 Kernel 稍后挂载时重新发布；具体 `allow`、`ask` 或 `deny` 仍由 Kernel 自己的策略文档决定。此外，该声明把这些工具标记为 `untrusted`：MCP 服务器位于信任边界之外，因此启用不可信内容隔离的部署要求这些工具先获得人工回答才能运行。
