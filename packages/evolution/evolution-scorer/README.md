...
`shouldOptimize(usage, thresholds)` is the pure trigger that decides whether one skill's recorded outcome warrants an optimization run. A curator-graded task outcome decides whenever the skill has any (`usage.sessionOutcomes`, amendment S9): the rate is `failed / graded`, gated by `triggerMinUses` graded sessions. Absent grading, it falls back to the tool-load counters: at least `triggerMinUses` loads with a failure share over `triggerFailureRate`, where the share is `failureCount / (useCount + failureCount)` — the rate the telemetry record documents. `evaluateSkill({ skill, scenarios, agent, run })` scores each named scenario through the existing `score` and aggregates the optimizer triple: `pass` holds only when every scenario passes, and tokens and wall time sum the per-scenario medians. One skipped scenario skips the whole evaluation with its reason, because optimizing on a partial evaluation would select on evidence that is not there; a skill naming no scenarios skips the same way.

### Behavior evaluation

`evaluateBehavior({ baseline, candidate, candidateBody, catalog, positiveQueries, negativeQueries, routingTopK?, vectors? })` judges one skill revision through three gates, cheapest first. The contract gate (`checkBehaviorContract`) refuses a body that would break the skill's frontmatter, reusing the exact invariant the skill manager enforces on edit. The routing gate (`checkBehaviorRouting`) runs positive and negative trigger queries through the real selector: every positive must rank the candidate inside `routingTopK` (default 3), every negative must keep it outside, over a catalog carrying each entry's revision key so a report never mixes revisions. Catalog entries may also carry frontmatter `requires`, `capabilities`, and `conflictsWith`, forwarded into the same selector call: a candidate whose prerequisites no catalog entry supplies — by name or as a capability another entry declares — scores zero and fails every positive, because routing to it could never work, and a candidate that loses to a better ranked entry it declares a conflict with cannot pass its positives either. A failed cheap gate stops the evaluation with `status: 'gated'` before any fresh process boots. Otherwise both replay compositions run over the same scenarios and the replay gate (`compareBehaviorReplay`) approves only when the candidate regresses nothing the baseline passed — parity on a scenario both fail is not a regression, because the gate judges "no worse" while selection judges "better". `approved` is true only when every gate passed: only replay evidence approves. Every judgment also carries `disagreement`: the channels reduced to approving and dissenting lists in canonical cheapest-first order, with `unanimous` true when they speak with one voice — agreement to reject counts as unanimous, so only a split is the uncertainty signal. On the `gated` path the reduction covers only the two cheap gates, because replay never ran; a lone passing gate beside a failing one still reads as a split, never as approval. `evaluatorDisagreement` is pure, so callers can reduce their own channel verdicts without spawning anything.

### Durable evidence (§24.3)

Every `ScoreRecord` — inside a `ScoreOutcome` and inside each `SkillScore.scores` entry — carries two fields toward SPEC §24.3's durable-evidence list, alongside the existing pass/tokens/wallTimeMs triple. `fixtureDigest` is the SHA-256 of the scenario's recorded fixture(s) — the primary session fixture plus every child fixture, in the scenario's own ordinal order — computed once per `score()` call, since the fixture does not vary across attempts; it names exactly which corpus generation the record measured against. `trajectory` is the first attempt's harvested session id (`ScoreAttempt.sessionId`, set from `result.sessionLogs[0].id`), or `null` when the runner harvested none — a deterministic reference a reader can pull the transcript from, without picking among a scenario's several attempts.
...
| `attempts` | `3` | Fresh-process attempts per score; the median is taken over their samples. A caller overrides this per call with `ScoreRequest.attempts` / `EvaluateSkillRequest.attempts` — `evolution-optimizer`'s S9 tier gate passes `1` for a body its own run started from, since the recorded fixture already validates it |
...
| [`src/disagreement.ts`](src/disagreement.ts) | Pure channel-verdict reduction into approving/dissenting lists with unanimity |
