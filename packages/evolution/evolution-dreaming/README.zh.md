...
把某个作用域反复失败的教训在三个仿照睡眠的阶段里固化为持久记忆：Light 对它汇集到的失败与情景笔记按陈述去重，REM 把两者共享的主题写成叙事，Deep 用六信号复合分给每个候选打分，只提升通过全部三道门限的候选。一条命令或空闲 heartbeat 就能跑完整个周期，`/dream <phase>` 单独跑一个阶段用于诊断。权重固定；阈值、节奏与保留量都是配置。提升结果是持久的，但还没有任何东西把它读回模型上下文，所以选它是为了固化，而不是为了召回。
