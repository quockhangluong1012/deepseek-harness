...
| `profile` | `default` | 作用域身份命名空间；必须与读取这些 dreams 的读取方所用 profile 一致 |
...
- **不可溯源**——记忆存储日常日志中的情景笔记。该层只记录笔记文本、日期与时刻，没有任何能标识作者的字段：模型抽取、用户本人与系统提示读起来完全一样。仅靠这类观察的候选会被按名拒绝（`unattributed-sighting`），无论它得多少分，因为存储既不记录文本来源，就没有任何闸门能为它作保。
...
记录本就作答的身份——规范陈述本身，或它已并入的某个措辞——不会写出第二条叙事，但会推进那条叙事的提升时刻，这正是该失败在作用域仍在记录它时保持持久的原因。每个候选都与同一轮已写入的叙事相比对，因此同时到达的两条复述只会产生一条叙事。

### 阶段分离

只有 deep 阶段写入持久记忆。light 与 REM 可以单独运行以供检视，而不改变该作用域已学到的东西；自动周期按顺序运行三个阶段。两个写入阶段都合并写入那一刻该域写入链上当前的记录，而不是该阶段先前读到的快照，因此重叠的 REM 叙事与 deep 遍历会同时留住各自的字段。情景笔记在记忆保留窗口内会反复进入 light，因此笔记带着衰减后的新鲜度被重新打分，而不是被记作「已消费」。提升存放在本插件自己的域中，绝不写入由模型拥有的经验文档，因此不会出现两个写入方争用同一份文档。

### 账本与回滚

每一轮只要新增、并入、退役或丢弃了叙事，就会写一条账本条目，条目同时持有这一轮替换掉的那份提升数组作为前像，以及它装入的内容。只推进了提升时刻的那一轮会写入记录而不写条目：它没有并入、退役或丢弃任何东西，因此回滚没有属于自己的可恢复内容，有限的账本也把空间留给真正移动过叙事的那些轮次。记录本身就是自己的 blob 存储，因此前像不可能在写入与读取它的回滚之间丢失；条目的 evidence 记录这一轮做了什么——新增、并入、退役、丢弃——以供审计。`rollback(scope, entryId)` 在写入任何东西之前先对未知身份大声失败，随后原样恢复前像，并把自身也记入账本，使回滚与被它撤销的那一轮同样可逆。账本由 `maxLedgerEntries` 限定，保留最新的若干条，因此长寿命作用域拥有的是一段回滚窗口，而不是它跑过的每一轮。
...
- **并入不会重置衰减时钟**——`staleAfterDays` 从提升时刻起算，而只有记录本就作答的候选（规范陈述，或它已并入的某个措辞）才会推进该时刻；新的复述会并入叙事而不推进它，因此一条比窗口更旧的叙事可能并入一条复述，又在同一轮被丢弃。沉寂很久之后的合并，更可靠的做法是让失败重新进入暂存，从而提升出一条新叙事。
