...
`dsh-evolution-router` learns which model route to use for which task class and evolutionary role. Each outcome records the route's pass, tokens, and wall time on a task class; effectiveness is derived at read time with running means and pass rates, and the recommendation ranks routes by a sample-confidence-adjusted score so a route with few outcomes cannot outrank a well-measured one. Nothing here calls a model.
...
- [Evolutionary Harness specification](../../../specs/evolutionary-harness-v11-deep-research.md) §28 — the adaptive model routing this package generalizes into per-task-class self-optimization.
