...
- **未内置领域模拟器**——`applyConsolidation` 可以向阶梯转发 `simulation`/`evaluator`/`review` 接缝，`requireVerifierPass` 也能要求完整通过，但本插件自己的通过不挂载任何接缝，`requireVerifierPass` 也保持默认的 `false`；这个仓库里也没有任何包实现可挂载的第 2 级领域模拟器。
- **受保护文本是显式的哨兵区段，而非内容感知**——正文中 `<!-- dsh:protected -->` … `<!-- /dsh:protected -->` 区段必须逐字节原样出现在补丁里，否则写入被跳过，记为 `level: 'protected-text'`；不携带哨兵的正文永不在这一关被拒绝，区段挪到文件里别的位置仍算存在。
