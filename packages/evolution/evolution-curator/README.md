...
Mounting once per host moves skills `active → suspect → stale → archived` on idle age and evidence, revives a `suspect` or `stale` skill whose newest load answers the evidence, and previews each pass as a dry run. `consolidate` has a model merge agent-created skills into umbrella skills; pinned, protected, bundled, and hub skills never move. Without skill telemetry a pass degrades to bookkeeping. Real passes write whole- or entry-reversible snapshots.

Mounting is the trigger: `enabled: false` registers no maintenance task and touches no bookkeeping, and automatic passes run on `ctx.evolutionHeartbeat`'s single schedule.
...
Mount the plugin once per host, with [`dsh-evolution-heartbeat`](../evolution-heartbeat/README.md) mounted alongside it. Background scheduling has one owner: the curator registers a single task, `skill-curation`, in the heartbeat registry, so the engine decides when the due-check runs. The registered task carries the configured `intervalHours` and `minIdleHours`; the engine observes host-wide `session/event` activity itself, seeds a newly registered task and defers one interval, and never runs two passes at once. The curator keeps its own activity listener because `maybeRun`'s idle gate reads it. Mounting without the engine is supported — nothing runs automatically, and `maybeRun`/`run` are the only triggers. `enabled: false` registers no task and leaves the bookkeeping untouched.

Call `maybeRun` to run the same due-check yourself (the idle gate takes an explicit `idleMs` override), or `run` for an unconditional pass, with `dryRun: true` to preview the report without writing. The first `maybeRun` seeds `lastRunAt` and defers one interval; the registered task calls `run` directly, because the engine already applies the interval and idle gates. A pass examines every tracked skill: idle age counts from the last load, or from seeding when never loaded. `active` moves to `stale` past `staleAfterDays`. Evidence moves a skill one rung more slowly than idleness does: every evidence-driven movement lands on `suspect`, which then ages into `stale` on the same idle threshold `active` does.
...
Intervals, thresholds, retention, and the consolidation route are validated `Config` members changeable from `cordis.yml`. An archive threshold below the stale threshold fails loudly, as does a half-set `provider`/`model` pair or an opted-in consolidation without both. `intervalHours` and `minIdleHours` are the cadence the registered heartbeat task runs under; the tick itself belongs to [`dsh-evolution-heartbeat`](../evolution-heartbeat/README.md).
...
|---|---|---|
| `enabled` | `true` | Master switch; off skips every pass, registers no task, and touches no bookkeeping |
...
| File | Role |
|---|---|
| [`src/index.ts`](src/index.ts) | Plugin entry: `EvolutionCurator` service, pass logic, host-wide activity listener and heartbeat registration, rollback, and bookkeeping |
...
A failing transition propagates and stops the pass: earlier movements stand, the report is discarded, and the bookkeeping stays unstamped so the next pass re-examines every skill. A failing pass reaches the registered heartbeat task, which records the failure and retries it on the interval; nothing is swallowed and the curator's own state stays unstamped. Invalid bookkeeping fails the domain open loudly: a lost `lastRunAt` would rerun the first-run deferral and shift the schedule. Reads throw before the curator starts.
...
Capped: at most `maxSteps` requests per run, each bounded by `maxInputBytes` of framed survey plus prior tool results, and `maxOutputTokens` of completion. With `ctx.evolutionBudget` mounted, the run also opens the deployment's daily and weekly consolidation ceiling first and settles every request's tokens and the run's wall time against both, so a spent ceiling buys no request at all.
...
- **Automatic passes need the heartbeat** — the curator registers its task in `ctx.evolutionHeartbeat` and mounts cleanly without it, but then nothing runs on a schedule: only on-demand `maybeRun`/`run` calls move skills.
...
- **No domain simulator built in** — `applyConsolidation` can forward `simulation`/`evaluator`/`review` seams to the ladder and `requireVerifierPass` can demand a full pass, but this plugin's own pass mounts no seams and leaves `requireVerifierPass` at its default `false`, and no package in this repo implements a level-2 domain simulator to mount.
- **Protected text is an explicit sentinel span, not content-aware** — a `<!-- dsh:protected -->` … `<!-- /dsh:protected -->` span in the body on disk must reappear byte-identical in a patch or the write is skipped with `level: 'protected-text'`; a body carrying no sentinel never refuses on this rung, and a span moved to a different position in the file still counts as present.
...
- **Adoption is one-way** — adopted skills keep a user-directed creation record; no operation returns them to agent-created.
