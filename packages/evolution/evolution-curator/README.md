...
`dsh-evolution-curator` runs the automatic, model-free half of skill lifecycle curation — idle skills move `active → stale → archived` over skill telemetry, with dry-run previews, first-run deferral, and idle gating — and the opt-in, model-driven half: consolidation of agent-created skills into umbrellas. Pinned skills, protected names, and bundled or hub sources never move. The plugin is mounted once per host and owns its own schedule: it observes host-wide session activity, runs one start-time due-check, and then ticks. Real passes write one tarball snapshot plus ledger entries, with fail-closed whole-run or single-entry rollback; manual adoption claims agent-created skills, and TTL purge removes archived skills with their directories. Without telemetry, automatic methods degrade to bookkeeping or empty reports.
...
Call `maybeRun` to run the same due-check yourself (the idle gate takes an explicit `idleMs` override), or `run` for an unconditional pass, with `dryRun: true` to preview the report without writing. A pass examines every tracked skill: idle age counts from the last load, or from seeding when never loaded. `active` moves to `stale` past `staleAfterDays`, `stale` moves to `archived` past `archiveAfterDays`. The report names every movement with its reason plus skip counts for pins, protected names, and excluded sources, and carries the pass identity and snapshot filename when a snapshot was written. `lastRunAt` reads the last pass instant for status surfaces.
...
A real pass with movements writes one tarball under the curator home (`snapshots/pass-<timestamp>-<id>.tar.gz`, pruned to `backup.keep`): staged before/after record pairs, resolvable skill directories, and a manifest. Unresolvable names land in the pass evidence instead of failing. One `pass` ledger entry plus one `transition` entry per movement append to the append-only JSONL ledger, with before/after record blobs stored content-addressed. `rollbackPass` and `rollbackEntry` restore lifecycle states — never patched bodies — after verifying every record, blob, and recorded package move up front, snapshotting current records first so each rollback stays reversible, and recording one `rollback` entry per skill or reversed move. Unknown ids, missing blobs, untracked skills, a relocated package whose recorded path is gone or whose original path is now occupied, and a missing store all fail before any write. A consolidation run snapshots the packages at their post-run layout, so its `move` entries are the record of the pre-run paths.

### Adoption and purge

`adopt` claims one agent-created skill into user-directed standing through the telemetry `markAdopted` write, recording one operator ledger entry; anything without background-review authorship rejects, and the movement is one-way. `purge` removes archived skills past `archiveTtlDays`: the skill directory goes first (record-only when unresolvable), then the record drops, then one operator ledger entry lands per skill. Pinned skills stay, a zero TTL purges nothing, and `dryRun` previews the removal list without writing. `passes` lists recorded passes newest-first for status surfaces and rollback picks. `surveyCandidates` lists agent-created skills with verdict evidence — routing, state, idle age, and use counters sorted by name — without writing; the keep/patch/consolidate verdict arrives separately.
...
- **Rollback restores states and moves, not patched bodies** — `patch` verdicts are ledgered for audit; `rollbackPass` restores lifecycle states and relocated packages, never the previous `SKILL.md` text.
