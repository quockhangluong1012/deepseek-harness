...
`dsh-evolution-adversary` 维护按技能持久化的对抗探针日志：每个弱点家族一个提示词或场景，覆盖 §45 的八个类别，从边缘情形、提示词注入到评估器博弈。每个探针记录它是否暴露了弱点、以及该弱点是否已修复；挑战指名下一个待探针的类别，全部覆盖后则轮换探针最少的那个。一旁是 §46 评估器博弈防御清单：六个可自动化的防御，其缺口显示还有哪些尚未落实。`defenses()` 把它们读作操作者的断言；`observedDefenses()` 则从真正记录了证据的各存储推导每项防御，并把无法观测的那些标为未观测。此包不调用任何模型。
...
`probe(input)` 将一条探针记为未修复；`setRepaired(probeId, repaired?)` 在弱点修复后将其标记为已修复，默认即已修复。`probes(skill?)` 按最新优先列出探针；`challenge(skill)` 按配置的下限指名下一个待探针的类别。`setDefense(defense, satisfied)` 设置清单中的一项防御；`defenses()` 按规范顺序渲染完整清单；`defenseGaps()` 列出尚未落实的防御。`observedDefenses()` 从已记录状态而非操作者的清单行推导同样这六项防御，按规范顺序给出。
...
清单行是操作者的断言；`observedDefenses()` 读取已挂载存储真正记录的内容，每项防御为满足、开放或未观测：

| 防御 | 观测来源 | 满足条件 |
|---|---|---|
| `multiple-evaluators` | `evolution-evaluator-strategy.strategies()` | 两个评估器都带有独立判定 |
| `hidden-holdout` | `evolution-benchmark.tasks('holdout')` | 至少一个能力持有受保护的留出任务 |
...
- [`dsh-evolution-evaluator-health`](../evolution-evaluator-health/README.zh.md)——其裁决一致性、批准漂移与漏报/误报跟踪正是防御清单所守护的姊妹存储。
- [`dsh-evolution-evaluator-strategy`](../evolution-evaluator-strategy/README.zh.md)——`multiple-evaluators` 防御正是从该存储的每评估器独立判定观测而来。
- [`dsh-evolution-benchmark`](../evolution-benchmark/README.zh.md)——`hidden-holdout` 防御正是从该存储的受保护 `holdout` 分区观测而来。
...
本存储只做记录：此处不调用任何模型，探针、修复与防御满足都是作为事实记录的操作者判断。挑战在读取时由历史推导，因此下限变更会重新排序下一个类别，而无需重写探针；未设置的防御合成开放行，因此清单从空存储即可安全渲染。`observedDefenses()` 刻意把两个面分开——`defenses()` 是操作者的断言，观测是已挂载存储的记录——其逐项防御的观测者表以防御联合为键，因此新增一项防御就不可能不给它配观测者。
