...
) => effectiveness({ provider, model, passRate, passes: Math.round(passRate * (extra.runs ?? 4)), ...extra })
...
describe('routeDisagreement', () => {
  ...
  it('ignores routes measured below the run minimum and other task classes or roles', () => {
    const rows = [
      rate('a', 'm1', 1, { runs: 2 }),
      rate('a', 'm2', 0, { runs: 5 }),
      rate('b', 'm3', 0.9, { runs: 5, role: 'candidate-generation' }),
      effectiveness({ provider: 'c', model: 'm4', taskClass: 'reader', passRate: 0, runs: 5 }),
    ]
...
describe('routeDisagreements', () => {
  it('reports one disagreement per task class and role, strongest gap first', () => {
    const rows = [
      ...
      rate('e', 'm1', 0, { runs: 1 }),
      effectiveness({ provider: 'd', model: 'm1', taskClass: 'reader', passRate: 1, runs: 5 }),
      effectiveness({ provider: 'd', model: 'm2', taskClass: 'reader', passRate: 0, runs: 5 }),
