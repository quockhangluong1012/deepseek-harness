...
- **The separation-of-duties check has no caller yet** — `checkDuties` refuses a promotion or a verdict whose judging identity is the producing one, and nothing consults it. The promotion paths this repository runs are `/canary promote <id>` in `dsh-command-evolution` and the actuator's rollout monitor, and neither records an identity for either role, so the refusal exists as the store's own answer until a path that knows who proposed and who reviewed calls it.
