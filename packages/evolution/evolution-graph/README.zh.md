...
- **被缓冲的文本有三条丢失途径**——自动巡扫在提取该作用域的那一次运行中消耗其缓冲，因此若进程在此之前重启，自上次运行以来观察到的文本会丢失；缓冲区超过 `maxInputBytes` 时，最旧的消息会被丢弃以腾出空间；而单条消息若大于整个预算则会被整体拒绝而非截断，因此它根本不会被缓冲，后续运行也无从触及。窗口受 `intervalHours` 与字节上限共同约束。
