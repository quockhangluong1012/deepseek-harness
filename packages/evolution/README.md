...
| [`evolution-model-routes`](evolution-model-routes/README.md) | Model routing: the durable history of which provider/model each evolutionary role ran on, with measured evidence, per-task-class effectiveness and ranking, the §44 disagreement signal, and the per-run separation-of-duties check | `ctx.evolutionModelRoutes` |
