...
Both scenario lists are validated before any model call: a name repeated inside either list, or listed in both, throws instead of running.
...
- **The holdout is only as private as its configuration** — the optimizer refuses a search scenario that is also a holdout scenario, but nothing stops an operator from listing every scenario in both places and calling it a split; the corpus partition is a deployment decision, not an enforced one.
