...
离线搜索更好的 SKILL.md 正文：传入技能、语料场景与分选身份，这次运行就从你配置的变异算子中取候选，先在短子集上筛选、再在隔离的 `DSH_HOME` 覆盖层下评分，检查私有 holdout，要求按场景结果的成对符号检验清过 `promotionConfidence`，用重复的成对比较确认优胜者，并把击败基线的 Pareto 优胜者分选出来——排序先看成本，再看档案新颖度。它按需运行，绝不在每轮路径上，每个选中的算子都要一次模型调用，并需要一份练到该技能的语料。没有任何东西直接写技能；批准分选条目才落地。
...
| `holdoutScenarios` | `[]` | 保留给晋级检查的语料场景，搜索期间永不评分；为空时，运行改为从该作用域的挖掘语料中抽取 holdout |
| `minedHoldoutCount` | `0` | `holdoutScenarios` 为空时，一次运行从挖掘语料中抽取的 holdout 场景数；`0` 既不挖掘也不抽取 |
| `maxMinedScenarios` | `50` | 一个作用域的挖掘语料保留的候选场景数，证据最强者优先 |
| `mineSignalLimit` | `100` | 一次挖掘所读取的轨迹会话与失败信号数量上限 |
...
| `promotionConfidence` | `0.95` | 成对符号检验必须达到的置信度，未达到就不称候选更好；晋级要求 `p <= 1 - promotionConfidence` |
...
只有一个提示词作为唯一变异机制会收敛到同一种改写风格，因此一次运行可以从一个算子组合中取候选。这个组合就是 [`dsh-evolution-operators`](../evolution-operators/README.zh.md) 拥有的算子词汇表，本包只导入它（`MUTATION_OPERATORS`，即带各自指令行的 `MUTATION_OPERATOR_CATALOG` 条目），因此算子存储的排序所看重的算子，正是某次运行能选到的算子：`rewrite`（清晰度与顺序）、`compress`（在保留证据指向的每条规则的前提下最短正文）、`guard`（补上缺失的前置条件、拒绝或校验）、`exemplify`（每条被证据指向的规则配一个实例）、`generalize`（把规则放宽到失败实例之外）、`decompose`（把流程拆成可核查的步骤）、`compose`（合并重叠规则）、`reorder`（把证据指向的检查提前）、`remove-step`（删掉从不触发的步骤）、`change-tool`、`change-retrieval` 与 `change-evaluator`（只替换工具调用、检索或自检）。`operators` 按请求顺序列出要用的 id；未知 id 在加载时失败。
...
- **凡是走到评估阶段的运行都会成为作用域 `evolution_experiments` 域中的一行持久记录**（schema 版本 5）：它所检验的假说——证据文本加上它提出的修复算子——证据文本、产出候选的算子、搜索与 holdout 场景、基线与优胜者三元组、置信度计数、支撑该判定的成对显著性比较（`significance`，未选出优胜者时为 `null`）、该次运行实际检查的 holdout、结果、原因、创建分选条目时的 staged id、被晋级的两臂共同所处的评估上下文、provider 与模型、挂载 scorer 本次测量所用的评分语义版本、被晋级正文相对起始正文的新增与删除行数、被晋级正文参与排序时的新颖度档案读数（`winnerArchiveNovelty`，未晋级时为 `null`），以及运行起始正文与被晋级正文的 SHA-256——足以让读者看出这次挑选由哪些轴决定。正文本身不落盘，由摘要标识；改动有多大由计数说明。`/curator experiments` 在晋级的行上把计数打印为 `+added/-removed`。还有四个字段用来补齐 SPEC §24.3 持久证据清单里这次运行实际测量到的部分：`fixtureDigest` 是优胜者（若无优胜者则是重评基线）每个场景分数背后所用录制夹具的合并 SHA-256，因此一行能准确点名它由哪一代语料验证过；`trajectory` 按场景顺序列出被评分场景的已采集会话 id，未采集到任何会话时为空；`policyProfile` 是请求 agent 的具名 `dsh` profile，若 bin 是自带配置语法的仅测试用假 bin 则为 `null`；`verifierOutput` 是被晋级正文经 frontmatter 契约门（这次运行自己唯一会咨询的验证器，比整理器更完整的阶梯窄）判定结果的 JSON，未晋级时为 `null`。SPEC §24.3 八个字段里有七个已这样绑定；只有任务契约还没有可接的来源，见「已知局限」。还有一个字段承载晋级所依据的内容：`evaluation` 是基线与获胜候选共同所处的评估上下文——基准标识、模型路由、预算、任务集，以及各臂实际买到的尝试次数——只在该次运行选出优胜者时记录。字段存在之前记录的行读作从未记录（`null` 或空），因此台账能在三个 schema 版本之间打开。
- **这一行就是实验登记表，按规范给出的键来命名。** `id` 是台账标识，`hypothesis` 是这次运行检验的理论，`baseline` 是运行起始正文的 `bodySha`，`variant` 是它晋级的正文的 `winnerSha` 与 `winnerOperator`，`benchmark` 是晋级所依据的 `evaluation.benchmark`，指标则是 `baseline` 与 `winner` 两个三元组。被记录的决定仍是 `outcome`——离线运行真正可能抵达的六种裁决之一——而不是规范那个五值状态联合：运行只在评估之后才记录，拒绝就是终点，因此 `draft`、`running`、`passed` 在这里没有生产者。把 `outcome` 当作那个状态读即可：`staged` 就是 `promoted`，其余一切已测量的裁决都是 `rejected`。
- **除非两臂以同样方式测量过，否则不会晋级。** 在做任何晋级工作之前，本次运行会构建重评基线与获胜候选各自的评估上下文，一旦两者在基准或任务集上不一致就拒绝继续——抛错，而不是分选一个猜测。基准是 scorer 版本加上两臂各自所读场景的录制夹具内容摘要，因此在两次评分之间重新生成的语料会读作另一个基准，而不是一个可比较的分数；任务集是各臂实际评分过的内容，按顺序。模型路由（每次尝试所启动的 agent 组合）与预算（本次运行自身的上限）是两臂共享的单一运行级选择，记录在同一个上下文里而不是参与比较。holdout 那一对在其支配判定之前也要过同一道检查；台账行携带的是搜索两臂共享的那个上下文。
- **血缘存储挂载时，晋级会落进策略链。** 被晋级的正文会通过 `ctx.evolutionLineage.recordRevision` 记录为 `skill:<name>` 的一个修订，由存储分配版本号、对正文取哈希，并与它所替换的修订做 diff，因此该技能的策略历史保持可版本化、可 diff、可复现、可回退（§14.5），而不需要本包再存一份正文副本。记录失败只记录警告，绝不会把已经发生的晋级变成错误。
...
- **`holdoutScenarios` / `minedHoldoutCount`**——受保护的 holdout，可配置也可挖掘。两者都为空时，运行拒绝分选任何内容：没有 holdout 结果就不晋级任何东西。设置 `minedHoldoutCount` 且未配置列表时，运行会先把该作用域已记录的失败信号挖掘进一个持久语料——其台账行记录过的轨迹会话（由 `dsh-evolution-trace` 压缩），加上 `dsh-evolution-feedback` 跨会话聚合的失败——再抽取该技能尚未搜索过、证据最强的候选；挖掘会先打开该技能自己的额度，额度已花完就不挖掘，台账记录的是抽出的名字而非空配置。抽到但该技能已搜索过的候选会被跳过，而不是交给污染检查去拒绝；该检查仍会拒绝任何被本次运行自己的台账记录为搜索的 holdout。配置后，任何内容被分选前，优胜者与基线都会先在其上评分，若基线在那里支配优胜者则拒绝（`status: 'holdout-rejected'`，不分选任何内容）。holdout 放在插件配置而非请求里是有意的：能点名 holdout 的调用方，也能丢掉一个没通过的场景。而该技能自己的台账已记录为搜索过的 holdout 场景，会在运行任何东西之前被拒绝：选出过早期优胜者的任务已经塑造了这个技能，它不再是干净的测试，改配置也洗不白它。
...
- **一级内容键为语料已经验证过的正文买下一次便宜的运行（S9 的一级）。** 每次评分先对 `(skill, scenarios, body)` 做哈希；若某个正文与本次运行自己出发的那个正文一致——也就是语料夹具录制时所针对的那个正文——就无视评分器配置的尝试次数，买下一次确定性重放尝试，因为重放固定夹具不会与自己产生分歧，而且墙钟时间本来就从不决定比较结果。任何变异出的候选都是内容键未命中，永远升级到评分器配置的完整尝试次数（二级）。该键在每次调用时都重新计算，从不缓存：一个正文测得的分数永远不会跨独立运行复用，因此晋级判断永远不会靠一个过期的读数支撑。
...
- 分选技能协议（approve 即以一次事务写入 patch 正文及其 preimage；rollback 恢复它）见 [`dsh-command-evolution`](../command-evolution/README.zh.md)。
...
- 本次运行组合所取材的算子词汇——以及按记录结果给这些算子排序的存储——是 [`dsh-evolution-operators`](../evolution-operators/README.zh.md)。
...
- **holdout 的私密性只取决于配置，但改配置洗不白它**——优化器既拒绝同时出现在搜索列表中的 holdout 场景，也拒绝该技能已经搜过的 holdout 场景；但没有任何东西阻止运维把每个场景在两处都列上还称之为切分；语料切分是部署决定，不是强制约束。挖掘读取的是作用域自身运行记录下来的失败信号，因此任何评分运行都没产生过的失败模式都进不了语料；而挖掘出的候选只有在同名语料夹具存在之后才能被评分——语料没有描述的名字是一次跳过，运行会把它报出来。
- **挖掘是失败信号的连接，不是夹具录制器**——它指出录制器应当为作用域实际命中的失败去录制哪些场景；它不去编写那些名字所解析到的录制重放夹具，而某个挖掘出的名字缺少夹具时，下一次运行报的是被跳过的场景，而不是 holdout 裁决。
...
- **SPEC §24.3 的任务契约字段没有可接的来源。** `TaskContract`（`dsh-agent-kernel`）是个真实类型，但 `/curator optimize` 运行在内核任务生命周期之外，且没有任何已发布 bundle 挂载 `agent-kernel`（见 §32），因此没有已创建的契约实例可引用——凭空造一个等于编造这次运行并不具备的内核集成；台账行为它保留任何字段都没有。相比之下 `verifierOutput` 是已绑定的，只是窄：它只记录这次运行本就要咨询、用来放行候选的 frontmatter 契约门，而不是整理器更完整的验证器阶梯（`runVerifierLadder`，见 `dsh-evolution-curator`）或 scorer 的路由/重放行为门（`evaluateBehavior`）——这两者本次调用图都不涉及：它们需要这条调用链不具备的路由目录与触发查询。只要存在优胜者，一行的 `verifierOutput` 就是 `{ok: true, issues: []}`，因为只有已经通过该门的正文才会成为候选。
...
- **台账保留自己的裁决词汇，而不是规范的状态联合**——记录的是 `outcome`，它的六个值区分了分选守卫、停滞窗口与重复拒绝所读的裁决（`staged`、`regressed`、`unconfirmed`、`holdout-rejected`、`no-improvement`、`skipped`）。把它们折成 `passed`/`rejected` 会丢掉这些区分，而那个联合剩下的三个状态在这里没有生产者，因此台账不携带它；想要粗略状态的消费者把 `staged` 映射为 `promoted`，其余一切已测量的裁决映射为 `rejected`。改动 `outcome` 的候选值同样是破坏性的记录变更：存储域没有迁移钩子，`compatibleVersions` 只接纳当前 schema 已能接受的旧行，因此被替换的联合会让既有台账拒绝打开。
- **「同一模型」指的是同一 agent 组合**——评估上下文记录的是每次尝试所启动的组合（具名 profile，否则就是配置路径），因为这个组合才是决定模型的东西，而优化器读不到它内部的模型标识。因此两个解析到同一模型的组合会被读作不同上下文并拒绝晋级，这是安全的方向；但在两次评分之间从某个组合配置内部改掉模型，这道检查看不到。
...
`lineage.ts` 推导晋级记录的两个事实——测量所支持的结论与两臂共同所处的评估上下文——而它所记录的行级 diff 来自 `evolution-lineage` 的 `lineDiff`，该函数归于血缘包以供策略修订链使用，因此纯粹的重排在那里记为有增有删；而 `novelty.ts` 比较的是忽略顺序、折叠大小写与空白的行集合，于是同一次重排记为 0——两个数字有意回答不同的问题，彼此都不是对方过时的副本。`instructionLines` 只剔除开头那一处 `---` 围栏块，别的都不动，因为 frontmatter 是每个候选都会照抄的路由元数据；其下没有指令行的正文报告的新颖度是 `0` 而不是 `1`，因为它没有任何可称新颖的内容。不满足 `experimentRecordSchema` 的持久行会让领域打开时大声失败，而不是被跳过，因为丢掉一条结果会掩盖台账声称已经发生的晋级。
