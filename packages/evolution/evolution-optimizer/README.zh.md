...
- **新颖度会被测出来，而「候选说了正文没有的东西」决定它的排序。** 每个候选正文都会与该次运行的起始正文相比较：`novelty` 是它那些互异指令行中起始正文尚未包含的比例——开头 frontmatter 会被剔除，因为每个候选都会照抄它——大小写与空白折叠、忽略行序，因此重新排版或重新排序记为 0，而技能从未声明过的规则记为一个真实比例。它出现在报告的每个候选上。选择时它作为「成本之下、墙钟时间之上」的并列打破依据，同时作用于筛选切分与优胜者挑选：在测得相同的候选之间，说出新东西的那个才会改变技能的行为，而全新回放进程上的墙钟时间只是机器噪声。支配关系仍然优先——新颖候选绝不会压过更便宜或更快的候选——所以新颖度只在「相等者之间」起作用，在本 harness 中即三元组完全相等，或筛选阶段 pass 与 token 相等。
