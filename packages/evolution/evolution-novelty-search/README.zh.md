...
`dsh-evolution-novelty-search` 维护跨运行的按技能行为描述符档案——每个暂存优化器写入对应一条条目——并测量每个新描述符相对该技能此前全部所见的新奇度。新奇度 = 1 − 与任意档案条目的最大 Jaccard 相似度：复述已知指令的候选不算新奇，携带新材料的候选才算。优化器通过可选存储接缝记录每次暂存写入，并按档案给每次运行的候选排序，宿主命令 `command-evolution` 通过 `/novelty` 检视档案。此包不调用任何模型。
...
优化器保留自己的按基线新奇度（`noveltyOf`）用于生成期比较，并与本档案共用并列打破依据：它每次运行只取一次 `entries(skill)` 快照，用 `archiveNovelty` 测量每个候选的描述符，因此只会复述档案正文的候选，会输掉它原本能凭正文新奇度或变异顺序拿下的并列。本包贡献的是持久化的档案新奇度信号——相对该技能全部历史而非单一参考体的差异度——并拥有这套测量；用哪些轴给候选排序、顺序如何，仍由优化器决定。
...
优化器通过可选存储记录，也只通过该接缝读取，因此未挂载该存储的运行在档案方面的行为完全不变；写入或读取失败只记警告，不会让优化失败。重复记录排除规则使同一候选标识的档案写入保持幂等。
