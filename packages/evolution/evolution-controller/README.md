...
| `setLessons` | `{ scopeId, artifacts }` | Replaces the whole lesson-artifact list and returns the projection; an artifact the caller omits is dropped rather than kept beside the new ones, and a supplied artifact that restates one the scope already holds keeps that record's counters, creation instant, source, and expiry. |
