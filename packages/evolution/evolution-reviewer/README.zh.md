...
在默认的 `defer: auto` 下，受门控的回合不再于 `turn/end` 立即提取，而是进入按会话的内存队列。刷出之前结束的回合会累积成同一份快照，而不是替换它：各回合的行按回合顺序追加，而第一份快照的 `deferMaxAgeMs` 截止时间与计时器保持不变，因此繁忙会话无法无限推迟自己的提取，超出上限的快照也只会因 `maxInputBytes` 丢掉最旧的行。会话释放会刷出其排队回合，而不是丢弃它们；插件 teardown 会丢弃它们，因为评审器自身的服务随其一起卸载。`defer: never` 恢复回合结束时的即时提取。
...
提取路由来自配置的 pair，否则取会话最近请求头；两者皆无的回合告警并跳过，无路由的重建直接拒绝。`error` 与 `aborted` 收尾抛入告警路径；`max-tokens` 以 `truncated` 容忍；工具调用块与其他收尾一律拒绝。模型调用使用 `temperature: 0` 与 `purpose: 'evolution-review'`，因此推理保持关闭，用量归属评审任务。挂载 `ctx.evolutionBudget` 时提取受预算门控：它在向模型发问之前打开该作用域的每日与每周额度，并在拿到回答后把其 token 与墙钟时间结算到两者之上；额度已花完时该调用被拒绝并告警——由 `openCeiling` 发出——已存经验保持不变，且不发任何请求。召回失败、批评条目被存储拒绝，以及召回条目被存储拒绝时都只告警，不让回合失败，原上下文保持不动。
...
带上限：每个受门控的回合或每次 defer 刷出一次辅助请求，受 `maxInputBytes` 笔录（含失败的工具结果）加 `relevantArtifactLimit` 个工件 statement、`maxOutputTokens` 补全约束。批评只是在同一份回答里多出几个 token，而不占一次请求。挂载 `ctx.evolutionBudget` 时，一次提取还会先打开该作用域的每日与每周额度，并把其开销结算到两者之上，因此额度已花完时根本不会买下任何请求。召回为每个被观测回合增加一次索引搜索，并至多向作用域记录添加一个上下文条目；其材料只经下一份简报抵达模型，被记录下来的批评也以同样方式——作为多出的一个上下文条目——抵达。
