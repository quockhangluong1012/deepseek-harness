...
`dsh-evolution-reviewer` 从实时会话提炼演进经验，且永不阻塞轮次：它缓冲当前轮次的事件，在 `turn/end` 索引产出文件，把该作用域目录下排序过的既往工作召回进简报，并排入一次受门控的提取——拿本轮次对照作用域产物中相关性受限的一片，回答 `confirms` / `contradicts` / `new` 决策，直接应用，或在 `writeApproval` 开启时暂存待批。`rebuild` 从历史把发现折入同一批产物。当作用域的经验需要跟随其会话的实际行为时选择本包；每个受门控轮次花费一次模型调用。
