...
| `/memory add <text>` | Stage the given text as one `appendInstructions` write and report `Staged remember '<id>' (<gist>). Approve it with '/memory approve <id>'.`; re-adding text that is still pending bumps that entry instead of parking a duplicate. Blank text is a usage error. |
| `/memory reject <id>` | Drop one entry without applying it and report `Rejected staged write '<id>'.`. |
| `/memory <anything-else>` | `Usage: /memory pending \| add <text> \| approve <id> \| reject <id>` — the grammar is fixed. |
...
| `/skills approve <id>` | Promote a staged patch in one transaction: write its body over the skill file, record the preimage it replaces in the skill's policy chain, and resolve the entry (`Promoted staged skill patch (<gist>): wrote <file> and recorded the preimage it replaces. Roll back with '/skills rollback <skill>'.`). A patch whose body would not parse as that skill's file, or a mount without the skill catalog or lineage store, is refused with the reason and nothing is written; a creation proposal is dropped as before, because a skill that does not exist yet is written by the gated `skill_manage` write; an id that is not a staged skill reports `No staged skill '<id>'.`. |
| `/skills rollback <skill>` | Restore the recorded preimage of a promoted body (`Rolled '<skill>' back to revision <n>, recorded as a new revision.`), or refuse when the file's current bytes match no recorded revision, or the revision records no preimage. |
...
| `/claims [query]` | List the scope's standing lesson facts most believed first as `<n> active claim(s):` followed by `- <statement> [confidence <x.xx>, <n> confirming / <n> contradicting check(s), evidence <fact/observation/inference>, newest <instant>]`; a query matching nothing reports `No active claim matching '<query>' in this scope.`, a corrected fact answers under its new wording, and an invalidated one is never listed. |
...
| `/benchmark admit-datasets <dir>` | Admit the §5.3 dataset corpus at `<dir>` as fresh tasks and report `Admitted <n> benchmark task(s) from <n> dataset(s), <n> duplicate(s) skipped.`; a directory that holds no dataset file admits nothing, and an unreadable one reports the loader's own error. |
...
| Approving a skill-kind entry through `/memory` | `Staged skill '<id>' (<op>) is decided by '/skills approve <id>': write the skill with skill_manage first, then approve there to drop the entry.` — the entry stays staged, and a staged `patch` is promoted by `/skills approve` instead. |
...
- **A promotion is one transaction, and a proposal still waits for the model's write.** Approving a staged `patch` writes the body, records the preimage it replaces, and resolves the entry together — a failure after the write restores the preimage, so the file and the recorded chain never disagree (amendment S9). Approving a creation proposal only drops it in the store, because a skill that does not exist yet has no body to write: the human prompts the write through the gated `skill_manage`, then approves, and `/memory approve` redirects skill-kind ids to `/skills approve`. Nothing is silently discarded or silently applied.
...
- **A promotion writes a patch, never a new skill** — `/skills approve` writes a staged patch's body over the existing skill file and records its preimage; a creation proposal still only drops the entry, because a skill that does not exist yet is written through the gated `skill_manage`. `/canary reject`/`rollback` amend the matching lineage outcome so the recorded verdict tracks reality, but no `/canary` verb verifies the live skill file against the staged body, and none writes it (amendment S9).
