...
- **A tool may opt out of the deployment default entirely.** `unboundedTimeout: true` on a `ToolDefinition` (e.g. a tool that blocks on a human response, which has no natural time bound) skips the wrapper altogether — neither its own `timeoutMs` nor `defaultTimeoutMs` applies, and the caller's own signal reaches the tool unwrapped.
