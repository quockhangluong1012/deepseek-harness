...
- **The tool declares its own budget.** `timeoutMs` lives on the tool's `ToolDefinition`, read from the registry (`ctx.tools.get(exec.name, exec.agent)?.timeoutMs`), so a mistyped tool name is impossible, undeclared tools delegate untouched, and no deployment-wide default deadline exists. `unboundedTimeout: true` states the same contract for a tool with no natural time bound (it blocks on a human answer): the wrapper arms no deadline and the caller's own signal reaches the tool unwrapped.
