...
export function serialize(
  ...
  // Maintenance purposes never pay for thinking: titles, workspace-memory
  // summaries, and evolution reviews read text, not reasoning traces.
  const maintenance = options.purpose === 'session-title' || options.purpose === 'workspace-memory' || options.purpose === 'evolution-review'
  const effort = maintenance ? 'off' : options.reasoningEffort ?? (connection.defaults.reasoningEffort ?? (connection.defaults.thinking === 'disabled' ? 'off' : 'high'))
