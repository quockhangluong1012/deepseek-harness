...
Mount the plugin with recovery routes in rotation order. On a declined failure the plugin records the failure against its route, skips open breakers and the just-failed provider, runs key rotation when installed, appends `llm/fallback`, stashes the route for the step's next attempt, and answers `{ kind: 'retry' }`. A downstream retry decision is always preserved, never superseded. Aborts and empty route lists decline without switching.
...
| `eligibleCodes` | all codes | Failure codes eligible for fallback; omission admits every code |
...
- **Process-local breaker** — breaker state resets on restart and coordinates no agents; success resets and cross-session breakers arrive separately.
