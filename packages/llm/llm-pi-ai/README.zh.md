...
解析模型还会把目录的每百万 token `cost`（含任何用量档位）带入 `LlmResolvedModelInfo.cost`，使消费方能依据路由该请求的同一批事实为已记录用量计价。费率全为零的条目——pi-ai 自己对「价格未知」的标记——完全不暴露 `cost`，而不是表现为免费模型。

### 经由 OpenCode Go 网关路由

路由键以 `opencode` 开头，或解析后的端点托管于 `opencode.ai` 的请求，会携带 `x-opencode-session`，其值为循环标记的会话 id。该值在多轮、恢复、压缩与重试中保持稳定，使每个会话拥有独立的网关路由亲和；网关要求每个推理请求都携带该 header。同名静态 `headers` 条目会让位于按会话的值，因为跨会话共用一个固定 id 会坍缩该亲和。其他提供方收不到会话 header。
