...
路由键以 `opencode` 开头，或解析后的端点托管于 `opencode.ai` 的请求，会携带 `x-opencode-session`，其值为 loop 传入的会话 id。该值在多轮、恢复、压缩与重试中保持稳定，使每个会话拥有独立的网关路由亲和；网关要求每个推理请求都携带该 header。同名静态 `headers` 条目会被按会话的值取代，因为跨会话共用一个固定 id 会坍缩该亲和。其他提供方收不到会话 header。
