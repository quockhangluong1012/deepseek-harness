...
`dsh-client-hmr` 让已打开的 Web 页面与 Host 插件图保持同步，并重载重建后的浏览器 bundle。普通插件的启停无需刷新页面或重启 Host 即可生效。代码重建会替换受影响插件并重置其组件状态。bundle 监视与传输无关：不含 SSE 通道的组合会挂载仅监视 row，并自行响应同样的重建。模型不会收到新的输入或输出。
