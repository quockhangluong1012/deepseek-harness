...
「新任务可选择模式」开关控制保存的用户默认值是否生效。关闭时使用部署默认值，重新开启则恢复用户偏好。选择健康定义作为默认值也会同步当前新任务页面的空白会话。Creator 入口开启一个使用 `cordis` preset 的新任务。新会话选择器不要求开发者工具：它只跟随宿主的可选策略（`agent-preset-registry` 的 `modeSelectionEnabled`，默认开启），而修改该策略的开关才是归入开发者工具的那一项偏好。
