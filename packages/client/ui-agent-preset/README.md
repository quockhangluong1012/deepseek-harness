...
The “Choose a mode for new tasks” switch controls whether the saved user default is active. Hiding selection uses the deployment default; showing it restores the user preference. Choosing a healthy default also synchronizes the blank session on the current new-task surface. Creator starts a new task using the `cordis` preset. The new-session picker needs no Developer tools: it follows the host's chooser policy (`agent-preset-registry` `modeSelectionEnabled`, on by default), and the switch that changes that policy is the one preference filed under Developer tools.
