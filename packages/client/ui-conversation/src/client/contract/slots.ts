...
export interface ConversationContentInputProps {
  ...
  hero: boolean
  /**
   * Whether a page occupies the center track. The page's band is the
   * blank-Session hero's position, so the preset chip travels with it while the
   * hero chrome itself stays down.
   */
  pageOccupied?: boolean | undefined
}
