...
The root slot composes the sidebar, conversation, right column, and two frame-wide seats. The sidebar spans 264–420px, defaults to 280px, and retains a 56px rail when collapsed; below 1024px it collapses automatically, and opening the right panel collapses a manually expanded sidebar. The right panel first opens at 45% of the viewport, then retains the user's pixel preference, capped at 70%. To protect 400px for the center, the frame first reduces the right panel to 300px, then reports insufficient room so its occupant closes it, and only then compresses the center further. Dragging has no transition delay; the right handle is absent while closed or fullscreen.

`shell.overlay` floats above every column in a click-through layer. `shell.page` instead shares the center track with the conversation: one page at a time draws over it, clipped to the track, so the navigation column keeps its width and stays usable. An unoccupied `shell.page` renders nothing and an occupant that renders null leaves the layer click-through, so the conversation underneath stays visible until a page takes over. The layer fixes no page geometry and takes no pointer events of its own: an occupant that needs the conversation's resident composer — the one editor a page drives instead of drawing a second input — holds that band open in its own layout, publishes the band's box as `--dsh-page-composer-top`, `--dsh-page-composer-left`, and `--dsh-page-composer-right` for the seat that docks into it, and opts pointer events back on for the regions it paints. The center column isolates its own stacking context, which is what confines the conversation's inner z-indexes beneath the page layer.
