...
describe('AppFrame', () => {
  ...
  it('renders owner props for the default sidebar and prospective right panel', () => {
    ...
    expect(slotCalls.find(c => c.key === 'main')).toEqual({ key: 'main', props: { pageOccupied: false }, options: { entryKey: 'conversation' } })
  })
  ...
  it('renders the page layer as a permanent sibling of the center column', () => {
    selectedSession = undefined
    const { frame, getByTestId } = mountFrame()
    // The layer states no geometry of its own: unoccupied it is click-through,
    // and an occupant that renders null leaves it click-through, so the slot
    // call happens whether or not a page is open.
    expect(frame.querySelector('[data-shell-page]')).not.toBeNull()
    expect(getByTestId('shell.page-content')).toBeTruthy()
    // The conversation underneath keeps its seat while a page may occupy the
    // track, which is what lets its composer stay live.
    expect(getByTestId('main-content')).toBeTruthy()
  })

  it('hands page occupancy to the main panel through its owner share', () => {
    pageOccupied = true
    const { getByTestId, slotCalls } = mountFrame()
    expect(slotCalls.find(c => c.key === 'main')!.props).toEqual({ pageOccupied: true })
    // The page draws over the column rather than replacing it.
    expect(getByTestId('main-content')).toBeTruthy()
    expect(getByTestId('shell.page-content')).toBeTruthy()
  })
  ...
  it('switches only the keyed main outlet when the active panel changes', () => {
    ...
    for (const panelId of ['panel-a' as MainPanelId, 'panel-b' as MainPanelId, null]) {
      ...
      expect(slotCalls).toEqual([{ key: 'main', props: { pageOccupied: false }, options: { entryKey: panelId ?? 'conversation' } }])
