...
挂载 `dsh-workspace-changes` 后，可以查看每个顶层轮次改动了哪些文件及每个文件的行数、打开任一列出文件在轮次起始与结束时的对比，并把工作目录的代码回退到任一轮次的起始状态。它在轮次起始与结束时对工作树做 git 快照并加以比较，同时把文件工具编辑过的每个文件整份复制下来，因此 git 忽略的文件也会计入；没有仓库或 git 时，只列出文件工具的编辑。摘要、对比与回退都留在 Host 上，直到该会话被销毁；Web 改动文件卡片与 `/rewind` 读取它们。
