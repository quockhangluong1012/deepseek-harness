...
describe('present', () => {
  ...
  it('ignores a different present definition in the calling agent scope', async () => {
    ...
    owner.ctx.tools.register(defineTool({
      name: 'present', description: 'Scoped replacement.',
      // The implicit parameter root is closed, so a callable replacement must
      // declare the argument the caller passes or validation rejects it first.
      parameters: { files: { type: 'array', required: true, items: { type: 'string' } } },
      output: {
        schema: {
