...
hooks 组让 agent 运行可以复用为 Claude Code 或 Codex 编写的 shell 钩子。把对应集成指向现有的 `hooks.json`，即可在会话开始、提示词到达、工具运行、请求权限或运行停止时执行受支持的 command 与 HTTP hook。这些钩子可以用模型可见消息阻止提示词或工具调用、向对话添加上下文、强制再执行一步或让运行停止。当你需要保留现有钩子配置时，选择本组；每项集成只支持其来源工具所记录的 hook 子集。
