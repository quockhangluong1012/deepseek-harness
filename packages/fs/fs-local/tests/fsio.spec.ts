...
describe('resolveLocalTarget', () => {
  ...
  it.skipIf(!fileSymlinks)('two paths to the same file via a symlink share one targetKey', async () => {
...
describe('probeNoFollow', () => {
  it.skipIf(!fileSymlinks)('reports symlinks without following them', async () => {
    const real = join(dir, 'real.txt')
    const link = join(dir, 'link.txt')
    await writeFile(real, 'hi')
    await symlink(real, link)
...
describe('listDirectory', () => {
  ...
  it('lists direct children in stable order without reading content', async () => {
    ...
    await symlink(join(root, 'missing-target'), join(root, 'broken-link'), process.platform === 'win32' ? 'junction' : 'file')
