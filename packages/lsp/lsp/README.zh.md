---
description: "LSP 能力 seam（ctx.lsp）：按文件扩展名选择提供方、五种规范化导航与诊断操作及结构化错误，供组合或扩展语言服务器查询的用户与维护者阅读。"
...
使用 `dsh-lsp` 让 agent 向语言服务器查询定义、引用、实现、悬停文档和文件诊断。查询按文件扩展名选择提供方，并返回规范化结果与结构化错误，因此更换后端不会改变模型请求。该服务只公开只读操作，不提供通用 JSON-RPC 逃生口。它须与 `dsh-lsp-stdio` 等提供方和 `dsh-tool-lsp` 等消费方组合；单独使用时不产生查询结果。
...
挂载语言服务器提供方与 `lsp` 工具，即可为 agent 提供文本搜索无法可靠提供的语义代码导航和文件诊断。本包定义这些包所注册的服务；它自身不定义 UI、工具或提供方。

### 何时选择

当部署需要由语言服务器支撑模型可见的代码导航或文件诊断时，选择此服务。它覆盖只读导航——定义、引用、实现与悬停——以及文件级诊断，不包含修改（重命名、code action、格式化）、symbol 列表或调用层级。该服务不依赖特定提供方：本地 stdio 服务器、远程服务器与沙箱原生提供方都以相同方式注册，因此更换后端不会改变模型看到的内容或请求方式。
...
每个查询都会针对一个源文件提出一个语义问题。四种基于光标的操作需要位置；文件级 `diagnostics` 不需要位置。结果是规范化位置、悬停内容或当前诊断列表，而不是原始协议载荷。
...
| `hover` | 该符号的规范化文档，或无结果 |
| `diagnostics` | 匹配的 `textDocument/publishDiagnostics` 通知中的文件当前诊断；有界等待到期时返回空列表 |

`findReferences` 始终包含声明，因此影响分析绝不会遗漏定义位置。协议上的光标位置是从零开始的 UTF-16；面向模型的工具接受从 1 开始的坐标并转换。服务器可能对无错误文件不发布诊断；在有界等待结束前，尚在分析文件的服务器与干净文件无法区分。
...
- **封闭操作集。** 五种操作的联合是封闭的——新增操作会通过编译强制要求同步修改 seam、提供方与工具。没有 JSON-RPC 逃生口。基于光标的请求需要位置，文件诊断不需要。
...
- **只读操作**——seam 从不应用编辑。诊断是有界的推送快照：服务器不发布诊断，或在等待结束时仍在分析，都会返回空列表。symbol 与调用层级需要不同 schema；修改操作需要单独工具，并集成预览、权限与写入策略。
