---
description: "The LSP capability seam (ctx.lsp): provider selection by file extension, five normalized navigation and diagnostic operations, and structured errors, for users and maintainers composing language-server queries."
...
Use `dsh-lsp` to let agents query language servers for definitions, references, implementations, hover documentation, and file diagnostics. Queries select a provider by file extension and return normalized results with structured failures, so backend changes do not alter model requests. The service exposes read-only operations only and has no generic JSON-RPC escape hatch. Combine it with a provider such as `dsh-lsp-stdio` and a consumer such as `dsh-tool-lsp`; alone it provides no query results.
...
Mount a language-server provider and the `lsp` tool to give agents semantic code navigation and file diagnostics that text search cannot reliably provide. This package defines the service that those packages register against; it defines no UI, tool, or provider of its own.

### When to choose it

Choose this service when a deployment wants model-visible code navigation or file diagnostics backed by language servers. It covers read-only navigation—definitions, references, implementations, and hover—and file-scoped diagnostics, while omitting mutations (rename, code actions, formatting), symbol lists, and call hierarchy. The service is provider-neutral: local stdio servers, remote servers, and sandbox-native providers register the same way, so replacing a backend does not change what the model sees or how it asks.
...
Each query asks one semantic question about a source file. The four cursor-based operations need a position; `diagnostics` is file-scoped and needs none. Results are normalized locations, hover content, or the current diagnostic list, never raw protocol payloads.
...
| `diagnostics` | The file's current diagnostics from a matching `textDocument/publishDiagnostics` notification, or an empty list after the bounded wait |

`findReferences` always includes declarations, so impact analysis never misses the defining site. Cursor positions are zero-based UTF-16 on the wire; the model-facing tool accepts one-based coordinates and converts them. A provider may publish no diagnostics for a clean file, and a server still analyzing a file is indistinguishable from that case until the bounded wait expires.
...
- **Closed vocabulary.** The five-operation union is closed—adding an operation is a compile-enforced change across the seam, providers, and tool. There is no JSON-RPC escape hatch. Cursor-based requests require a position; file diagnostics do not.
...
- **Read-only operations** — the seam never applies edits. Diagnostics are a bounded push snapshot: a server that publishes nothing, or is still analyzing when the wait expires, yields an empty list. Symbols and call hierarchy need separate schemas; mutations need separate tools with preview, permission, and write-policy integration.
