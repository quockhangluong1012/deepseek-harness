...
Ceilings are guardrails, not reduction targets. Aim for 5% headroom below target; tighter files only report. Above target, freeze the ceiling until relocation or condensation brings the document under target. Targets: root `AGENTS.md` ≤ 1,950 and ≤ 40 top-level rules; `architecture.md` ≤ 2,400; subtree `AGENTS.md` ≤ 600, except `packages/AGENTS.md` ≤ 750, `packages/client/AGENTS.md` ≤ 3,470, and this file ≤ 1,320; `packages/README.md` ≤ 994; plus `cordis-primer.md` 600, `defensive-patterns.md` 550, `testing.md` 1,350, every subtree `AGENTS.md`, and `.agents/notes/README.md`.
