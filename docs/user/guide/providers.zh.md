...
通过订阅而非 API 密钥认证的提供商，在卡片上提供**登录**控件而不索要密钥。例如 Anthropic 卡片经浏览器登录 Claude Pro 或 Max 订阅：选择**登录**，打开登录页面，把显示的验证码粘回去。授权与 API 密钥一起存在 `$DSH_HOME/.credentials.yaml` 中，settings 不保留任何敏感内容。已登录的路由显示已登录态与**退出登录**操作；退出只在本地忘记授权，不通知发行方。

关闭对话框即撤回正在运行的尝试。一次登录运行时，第二页打开同一提供商会被拒绝，直到前者结算；登录中途重载页面即放弃本次尝试。
