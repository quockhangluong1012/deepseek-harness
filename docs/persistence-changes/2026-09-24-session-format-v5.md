...
English | [中文](2026-09-24-session-format-v5.zh.md)

## Summary

Records the Session V5 header transition for the max-steps turn-end reason and the persistence types carried by this writer.
...
V4 readers cannot interpret turn/end.reason.kind=max-steps. V5 advances SessionHeader.version from 4 to 5 and adds the adjacent V4-to-V5 edge. That edge preserves V4 events and the inherited cut; the V5 codec retains V4 physical row encoding. The additive event roots and optional fields do not themselves require a version bump. New message source kinds are explicitly qualified as attribution-only, so readers preserve their JSON metadata without granting authority or adding producer-specific replay behavior. Committed predecessor generations remain unchanged, and no downgrade path is promised.

<a id="verification"></a>
## Verification

pnpm exec vitest run packages/session/session-format-v4-to-v5/tests: 3 tests passed. pnpm exec vitest run packages/session/session-format-catalog/tests: 35 tests passed. pnpm exec vitest run packages/session/session-persistence-jsonl/tests/v3-restart-migration.spec.ts: 4 tests passed. pnpm exec vitest run packages/session/session-persistence-jsonl/tests/catalog-migration.spec.ts: 82 tests passed, 1 skipped. pnpm exec tsc -b packages/session/session-format-v4-to-v5/tsconfig.json packages/session/session-format-catalog/tsconfig.json packages/session/session-persistence-jsonl/tsconfig.json --pretty false: passed. pnpm run verify-persistence-formats: passed.
