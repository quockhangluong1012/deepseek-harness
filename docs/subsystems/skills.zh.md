...
面向模型的 `skill({ name })` 工具校验 kebab-case 名称，在与调用策略无关的目录中查找摘要，并在加载前通过 `isModelInvocable` 拒绝无权访问的 skill；随后它根据调用方 agent 的 cwd 重新读取完整定义，并在返回内容前再次检查策略。该工具将无法解析的 skill 报告为未知或已不可用，并返回包含 `<skill_content name="...">`、`<skill_resources>` 和 `<skill_instructions>` 的工具结果。frontmatter 声明了 `requires` 的 skill 按其声明的组合加载：会话能够路由的每一项前置——按名称，或由某个以 capability 提供它的 skill——都作为紧随其后的一个 `skill_content` 块；若某项前置不可用，或该集合内部存在声明的 `conflicts_with`，则拒绝加载并指明原因。`resourceBase` 仅按需解析显式引用的脚本、参考资料和资产；加载结果不枚举 skill 目录。因此，仅修改正文会改变后续工具调用，而不会生成目录消息或改写先前工具结果。
