...
In development, [dsh-client-hmr](../../packages/client/hmr/README.md) is the registry's watch driver: its node half stat-polls every graph row's bundle from the module host's pre-read baseline, calls `rebuilt(id)` only for a changed or dirty row, resyncs its watch set through `onGraphChanged`, and broadcasts rev changes to the browser half over SSE. A carrier without that SSE channel — the Electron Desktop Host — mounts the watch-only `@deepseek-ai/dsh-client-hmr/watch` row instead and reacts to `onRebuilt` itself. Source-map changes alone do not trigger a reload; the current map joins the snapshot when a bundle change does. Production graphs omit both rows; the module host itself never watches files.
