...
- Modify/extend: `packages/evolution/evolution-reviewer/tests/reviewer.spec.ts` (or the equivalent existing test file covering `runExtraction`/`rebuild`/`storeDocument` — confirm the exact name from the directory listing)
