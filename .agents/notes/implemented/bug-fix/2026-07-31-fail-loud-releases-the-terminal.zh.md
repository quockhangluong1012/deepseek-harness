...
这些基于假进程的测试无法观测到最关键的两种失败形态——真实事件循环下的进程退出码，以及退出之后的终端状态——因此回归用例也放在 `apps/cli/tests/built-bin.e2e.ts`。它用 `fixtures/invalid-provider.cordis.yml`（`providers` 为列表形状，正是用户真实会犯的错误）启动构建产物，期望退出码为 1 且 stdout 为空，并断言 stderr 中带标签的启动 rejection（`plugin tree failed to load`）。已退役的 PTY 冒烟曾端到端钉住同一条启动路径，包括终端重置：正是它发现了以 13 静默退出、终端状态未被恢复的 [HMR（热模块替换）初始扫描启动死锁](../../archived/bug-fix/2026-08-03-hmr-initial-scan-boot-deadlock.md)。
