...
### Why a new package, not an extension of `dsh-evolution-memory-context`

Two independently evolving triggers: one reacts to a memory *write* (digest changes), the other reacts to *turn content* (every turn, regardless of whether anything was ever written to `evolutionMemory`). Two independent data sources: static per-scope curated fields versus raw session-history search. Folding both into one package would mean one file juggling two unrelated cache-invalidation stories — a violation of "one clear purpose" per capability seam.
