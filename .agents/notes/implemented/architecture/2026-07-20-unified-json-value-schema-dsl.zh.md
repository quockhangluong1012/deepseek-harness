...
显式的作者侧对象必须声明 `additionalProperties: true | false`。原始 JSON Schema 保留标准的默认开放语义；隐式参数根对象自[审计修复](../bug-fix/2026-09-09-harness-audit-remediation-signal-validation-approval-sandbox.zh.md)起编译为封闭（`additionalProperties: false`），因此未声明的模型键会以 `INVALID_ARGS` 失败。schema 记录只能包含自有且可枚举的字符串键，schema 数组必须是稠密的内建数组，系统只从自有属性读取受支持的关键字；因此，自定义原型、继承的约束、symbol 和 JSON 不可见的附加内容都无法让编译、投影和校验观察到不同的声明。内建的普通 Object 和 Array 容器跨 JavaScript 运行域后仍视为普通容器，而子类和伪造构造函数的原型仍视为非普通对象。
