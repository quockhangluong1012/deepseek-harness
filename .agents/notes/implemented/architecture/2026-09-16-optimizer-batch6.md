...
**Pareto is pure and strict.** Dominance on (pass, tokens, wallTimeMs); the winner must dominate the baseline, tie-broken by tokens, then the most novel body, then wall time, then mutation order. Novelty (`src/novelty.ts`, §31) is the share of a candidate's distinct instruction lines the starting body does not already carry, measured with case, spacing, and line order folded away; it sits above wall time because wall time over fresh replayed processes is machine noise while novelty is the axis that changes what the skill does. Dominance still rules above it: a novel candidate is never picked over a cheaper or faster one, so novelty decides between candidates that measured the same — an exact triple tie, or a screen tie on pass and tokens. Anything else — empty mutation answer, unbeaten baseline, corpus skip — reports `no-improvement` or `skipped` with a reason and stages nothing.
