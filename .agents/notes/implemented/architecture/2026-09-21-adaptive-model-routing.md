...
4. **One durable domain, one command surface.** The `evolution_model_routes` domain (v1) holds `routes` (keyed by role+route), `evidence` (keyed by outcome id), and the `duties` table (keyed by run+role) that §53's separation of duties reads (`2026-09-22-separation-of-duties.md`). `/routes` lists per-role assignments with their recommended routes, pins one, or reads the newest evidence. The package is mounted in the web-app profile (it writes only its own domain and reaches no model prompt).
