...
The adapter calls pi-ai's `streamSimple()` so each catalog model chooses its registered API implementation, including OpenAI Responses instead of Chat Completions where the descriptor says `openai-responses`. Harness temperature, maximum tokens, signal, session id, and the profile's common stream options flow through directly. Profile headers merge with the mandatory Harness-owned headers — attribution plus the per-conversation OpenCode gateway session header — with Harness-owned names winning. The adapter no longer maintains DeepSeek-specific payload rewrites or a provider-protocol matrix.
