...
- `evolution-graph` stored entities and directed relations, and a relation is *how often one phrase was read from one text*: it carries no provenance, so nothing could be contradicted. Its README recorded the consequence as a current limit — "an edge that a later source contradicts keeps its count".
