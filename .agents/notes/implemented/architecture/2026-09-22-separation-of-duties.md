...
4. **The refusal is the store's own check, and it is documented as unenforced.** `checkDuties(runId, decision)` is that rule over the store's own rows. No promotion path in this repository consults it: the two paths that take a promotion decision are `/canary promote <id>` in `dsh-command-evolution` and the actuator's rollout monitor (`canary.advance(id, 'promoted')`), and neither records an identity for either role. Rather than pretend to gate a path this package does not own, the README's limitations state that the refusal has no caller yet.
...
- The refusal is advisory until a promotion path that knows who proposed and who reviewed calls it. Until then §58.12's record-not-enforced boundary holds for this mechanism too, and a deployment that never calls `recordDuty` gets `unknown-identity` for every run rather than a guessed verdict.
