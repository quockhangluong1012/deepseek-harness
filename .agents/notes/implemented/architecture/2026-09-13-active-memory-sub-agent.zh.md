...
### 为什么是新包，而不是扩展 `dsh-evolution-memory-context`

两个独立演进的触发条件：一个响应记忆*写入*（摘要变化），另一个响应*轮次内容*（每一轮都响应，无论 `evolutionMemory` 里是否写过任何东西）。两个独立的数据源：静态的按 scope 精选字段，对比原始的会话历史搜索。把两者揉进一个包，意味着一个文件要兼顾两套互不相关的缓存失效故事——违反了能力接缝"一个清晰目的"的原则。
