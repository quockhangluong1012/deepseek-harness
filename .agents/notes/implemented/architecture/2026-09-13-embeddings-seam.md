...
Adding the semantic method to the definition is a breaking change to a live service: one production subclass (`SqliteSessionQueryEngine`) and seven test doubles across seven packages subclass `SessionQueryEngine`, and every one of them must implement the new method in the same change. Until the consumer exists, the two packages are correct in isolation but the capability has no in-repository caller.
