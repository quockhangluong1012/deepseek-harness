...
- **Recording which corpus each row scored.** Rejected: one host scores against one `corpusDir`, and ledgers never cross hosts, so the scenario name plus scorer version plus samples already identifies the measurement. A corpus field would stamp a constant.
