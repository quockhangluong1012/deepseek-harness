...
A bare `/memory` or `/skills` reports the pending list. `pending` is the grammar's default verb rather than a required one, matching bare `/journey`'s `7d`: a reader who reaches for the command with no arguments reads state, and a client whose menu picks a command without arguments — the composer dispatches a bare token — gets the same listing instead of a usage refusal.
...
`packages/evolution/command-evolution/tests/command-evolution.spec.ts` pins the commands through the real registry: registration and disposal of all five commands, usage errors for unknown and malformed verbs with bare `/memory` and `/skills` listing pending, the scopeless refusal, a seeded `/journey` rendered exactly (with the day key computed from `dayKeyUTC7`), the `/skills` empty state and its skill-only listing, approval dropping the entry, the cross-kind refusals on both commands, the `diff` gap message, `/curator status` with and without telemetry and with recorded passes, and `/curator run` in both modes with its skip counts, snapshot line, singular/plural wording, and unmounted refusal.
