...
`packages/client/ui-evolution/tests/` covers the host face (mounted and unmounted curator), the Remote face (every bound verb's request shape, failure unwrapping, one baseline per generation, the double-baseline guard, stream failure, idle-until-abort), the page (buckets with every delta kind and both decision counts, decisions retiring their row, capacity bars, range switching, follow baseline/upsert/failure, no-Scope/loading/empty states), the seats, and the plugin registrations. `packages/client/ui-evolution/tests/rpc.client.spec.ts` covers the Remote face over the fixture transport (every bound verb's request shape, baseline follow and upsert, the double-baseline guard, stream failure, and the idle-until-abort); `curator-status.host.spec.ts` covers the curator status face, and `page.client.spec.tsx` covers the timeline windows. Per-file coverage on this package's `src` tree is 100%.
