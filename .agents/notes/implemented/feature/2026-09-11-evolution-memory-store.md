...
- **Per-entry lesson creation record and history.** Rejected for v1, as with workspace memory: one capped document per tier stays directly editable, and the substring edit ops are the only touch points for a future array form.
