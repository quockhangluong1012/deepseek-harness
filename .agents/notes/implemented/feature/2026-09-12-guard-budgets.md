...
`maxCostUsd` prices the same measurement at the deployment's `usdPerMillionTokens` per million tokens. The harness has no pricing source of its own, so the deployment names what its model costs: a cost ceiling without a price fails load, and a price without a cost ceiling is inert. `maxTotalTokens` is also the honest name for what is compared, because `ctx.tokenMeter` measures total request pressure rather than input tokens alone — the specification table's `maxInputTokens` names a quantity the meter does not produce.
