...
- `@deepseek-ai/dsh-workspace-memory` (workspace group) owns the durable record on the `workspace_memory` domain (`per-record`, version 1, fail-loud on corrupt records): description, instructions, memory with `memoryUpdatedAt`, newest-last context items, newest-first outputs capped by `maxOutputs`, and `lastExtraction` recording which extraction wrote the document. Reads are synchronous; `capacityBytes` is required; every other cap is a validated `Config` field. Capacity charges instructions plus memory plus item sizes; the digest covers only those three, so description and output writes never re-inject.
...
- **Per-entry memory with a creation record and history.** Rejected for v1: a single capped markdown document needs no second storage tier and stays directly editable; the card, `setMemory`, and extractor merge are the only touch points for a future array form.
